...
public class JdbcGBase8aIT extends AbstractJdbcIT {
    ...
    private static final String GBASE_URL = "jdbc:gbase://" + HOST + ":%s/gbase?useUnicode=true&characterEncoding=UTF-8&rewriteBatchedStatements=true";
    ...
    GenericContainer<?> initContainer() {
        GenericContainer<?> container = new GenericContainer<>(GBASE_IMAGE)
            .withNetwork(NETWORK)
            .withNetworkAliases(GBASE_CONTAINER_HOST)
            .withLogConsumer(new Slf4jLogConsumer(DockerLoggerFactory.getLogger(GBASE_IMAGE)));

        container.setPortBindings(Lists.newArrayList(String.format("%s:%s", GBASE_PORT, GBASE_PORT)));
    ...
    protected void createSchemaIfNeeded() {
        ...
        try {
            ...
            throw new SeaTunnelRuntimeException(JdbcITErrorCode.CREATE_TABLE_FAILED, "Fail to execute sql " + sql, e);
        }
    }
}
