...
public class JdbcMysqlMultipleTablesIT extends TestSuiteBase implements TestResource {
    private static final String DRIVER_URL = "https://repo1.maven.org/maven2/com/mysql/mysql-connector-j/8.0.32/mysql-connector-j-8.0.32.jar";
    ...
    private static final List<String> SOURCE_TABLES = Arrays.asList(SOURCE_DATABASE+".source_table_1", SOURCE_DATABASE+".source_table_2");
    private static final List<String> SINK_TABLES = Arrays.asList(SINK_DATABASE+".sink_table_1", SINK_DATABASE+".sink_table_2");
    ...
    @TestTemplate
    public void testReadingMultipleTables(TestContainer container) throws IOException, InterruptedException {
    ...
    private void initSourceTablesData() throws SQLException {
        String columns =
            Arrays.stream(TEST_DATASET.getLeft()).collect(Collectors.joining(", "));
        String placeholders = Arrays.stream(TEST_DATASET.getLeft()).map(f -> "?").collect(Collectors.joining(", "));
