    ...
    public void testChangelogLookup(TestContainer container) throws Exception {
        ...
        futures.add(
                CompletableFuture.runAsync(
                        () -> {
                            try {
                                container.executeJob("/changelog_paimon_to_paimon.conf");
                            } catch (Exception e) {
        ...
        // cancel stream job
        container.executeExtraCommands(
                new CancelStreamJob("changelog_fake_cdc_sink_paimon_case1_insert_data.conf"));
        container.executeExtraCommands(
                new CancelStreamJob("changelog_fake_cdc_sink_paimon_case1_update_data.conf"));
        container.executeExtraCommands(new CancelStreamJob("changelog_paimon_to_paimon.conf"));
    ...
    public void testChangelogFullCompaction(TestContainer container) throws Exception {
        CompletableFuture<Void> voidCompletableFuture =
                CompletableFuture.runAsync(
                        () -> {
                            try {
                                container.executeJob("/changelog_fake_cdc_sink_paimon_case2.conf");
                            } catch (Exception e) {
                                throw new SeaTunnelException(e);
                            }
                        });
        ...
        // cancel stream job
        container.executeExtraCommands(
                new CancelStreamJob("changelog_fake_cdc_sink_paimon_case2.conf"));
