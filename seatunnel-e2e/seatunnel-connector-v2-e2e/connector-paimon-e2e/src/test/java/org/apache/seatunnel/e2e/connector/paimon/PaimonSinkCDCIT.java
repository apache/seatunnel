...
public class PaimonSinkCDCIT extends AbstractPaimonIT implements TestResource {
    ...
    //                        .getStderr()
    //                        .contains(
    //                                "[Paimon: The source filed with schema 'name INT', except
    // filed schema of sink is '`name` INT'; but the filed in sink table which actual schema is
    // '`name` STRING'. Please check schema of sink table.]"));
    //    }
