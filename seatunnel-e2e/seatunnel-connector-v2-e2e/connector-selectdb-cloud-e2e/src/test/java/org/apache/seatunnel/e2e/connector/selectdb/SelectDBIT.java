...
public class SelectDBIT extends AbstractSelectDBIT {
    ...
    @TestTemplate
    public void testSelectDB(TestContainer container) throws IOException, InterruptedException {
        initializeJdbcTable();
        batchInsertUniqueTableData();

        Container.ExecResult execResult = container.executeJob("/selectdb_source_and_sink.conf");
        Assertions.assertEquals(0, execResult.getExitCode());
        checkSinkData();

        batchInsertUniqueTableData();
        Container.ExecResult execResult2 =
                container.executeJob("/selectdb_source_and_sink_2pc_false.conf");
        Assertions.assertEquals(0, execResult2.getExitCode());
        checkSinkData();

        batchInsertDuplicateTableData();
        Container.ExecResult execResult3 =
                container.executeJob("/selectdb_source_to_doris_sink_type_convertor.conf");
