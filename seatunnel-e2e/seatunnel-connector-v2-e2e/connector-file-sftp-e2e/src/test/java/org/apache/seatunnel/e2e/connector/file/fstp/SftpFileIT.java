...
public class SftpFileIT extends TestSuiteBase implements TestResource {
    ...
    public void testSftpBinaryUpdateModeDistcpWithNonRecursiveScan(TestContainer container)
        ...
        try {
            putSftpFile(
                    SFTP_CONTAINER_HOME + "/tmp/seatunnel/update/src/root.bin", "root-updated-v2");
            putSftpFile(
                    SFTP_CONTAINER_HOME + "/tmp/seatunnel/update/src/subdir/nested.bin",
                    "nest-updated-v2");
            putSftpFile(
                    SFTP_CONTAINER_HOME + "/tmp/seatunnel/update/dst/root.bin", "root-stale-v1");
            putSftpFile(
    ...
    /**
     * Verifies strict checksum mode keeps nested SFTP files untouched during non-recursive scans.
     */
    @TestTemplate
