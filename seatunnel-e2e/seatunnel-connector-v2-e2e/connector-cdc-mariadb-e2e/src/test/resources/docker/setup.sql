...
CREATE USER IF NOT EXISTS 'mariadbuser'@'%' IDENTIFIED BY 'mariadbpw';
GRANT ALL PRIVILEGES ON *.* TO 'mariadbuser'@'%';

CREATE USER IF NOT EXISTS 'st_user_source'@'%' IDENTIFIED BY 'mariadbpw';
GRANT SELECT, RELOAD, SHOW DATABASES, REPLICATION SLAVE, REPLICATION CLIENT, DROP, LOCK TABLES ON *.* TO 'st_user_source'@'%';

CREATE USER IF NOT EXISTS 'st_user_sink'@'%' IDENTIFIED BY 'mariadbpw';
