...
public abstract class EdgeAgentTestSuiteBase {
    ...
    private TestContainersFactory containersFactory =
            () -> {
                List<TestContainer> discovered = ContainerUtil.discoverTestContainers();
                List<TestContainer> result = new ArrayList<>(discovered.size());
                for (TestContainer container : discovered) {
                    if (container instanceof SeaTunnelContainer
                            && EngineType.SEATUNNEL.equals(
                                    container.identifier().getEngineType())) {
                        result.add(new EdgeAgentSeaTunnelContainer());
                    } else {
