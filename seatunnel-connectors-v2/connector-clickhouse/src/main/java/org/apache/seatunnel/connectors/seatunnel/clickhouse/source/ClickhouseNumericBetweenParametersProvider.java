 ...
 */
public class ClickhouseNumericBetweenParametersProvider {
     ...
     */
    public ClickhouseNumericBetweenParametersProvider(BigDecimal minVal, BigDecimal maxVal) {
        checkArgument(minVal.compareTo(maxVal) <= 0, "minVal must not be larger than maxVal");
        this.minVal = minVal;
        this.maxVal = maxVal;
    }

    public ClickhouseNumericBetweenParametersProvider ofBatchNum(int batchNum) {
                    ...
                    start.add(BigDecimal.valueOf(batchSize))
                            ...
                            .subtract(
                                    BigDecimal.valueOf(i).compareTo(bigBatchNum) >= 0
                                            ? BigDecimal.ONE
                                            : BigDecimal.ZERO);
            if (i == batchNum - 1 && end.compareTo(maxVal) > 0) {
                end = maxVal;
            }
