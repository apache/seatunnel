...
package org.apache.seatunnel.connector.doris.sink.writer;

import static org.apache.seatunnel.connector.doris.sink.LoadStatus.LABEL_ALREADY_EXIST;
import static org.apache.seatunnel.connector.doris.sink.LoadStatus.SUCCESS;
import static org.apache.seatunnel.connector.doris.sink.writer.LoadConstants.LINE_DELIMITER_DEFAULT;
import static org.apache.seatunnel.connector.doris.sink.writer.LoadConstants.LINE_DELIMITER_KEY;
import static org.apache.seatunnel.connector.doris.util.ResponseUtil.LABEL_EXIST_PATTERN;
...
import org.apache.seatunnel.connector.doris.rest.models.RespContent;
import org.apache.seatunnel.connector.doris.sink.HttpPutBuilder;
import org.apache.seatunnel.connector.doris.util.ResponseUtil;
...
public class DorisStreamLoad implements Serializable {
    ...
    public DorisStreamLoad(String hostPort,
        ...
        this.executorService = new ThreadPoolExecutor(1, 1,
                ...
                new LinkedBlockingQueue<>(), new ThreadFactoryBuilder().setNameFormat("stream-load-upload").build());
            ...
            try {
                ...
                if (LABEL_ALREADY_EXIST.equals(respContent.getStatus())) {
                    ...
                    if (JOB_EXIST_FINISHED.equals(respContent.getExistingJobStatus())) {
                        throw new DorisConnectorException(DorisConnectorErrorCode.STREAM_LOAD_FAILED, "Load status is " + LABEL_ALREADY_EXIST + " and load job finished, " +
                    ...
                    if (matcher.find()) {
                        ...
                        throw new DorisConnectorException(DorisConnectorErrorCode.STREAM_LOAD_FAILED, "Load Status is " + LABEL_ALREADY_EXIST + ", but no txnID associated with it!" + "response: " + respContent);
                    }
                } else {
    ...
    public RespContent handlePreCommitResponse(CloseableHttpResponse response) throws Exception{
        ...
        throw new DorisConnectorException(DorisConnectorErrorCode.STREAM_LOAD_FAILED, response.getStatusLine().toString());
    }
    ...
    public void abortTransaction(long txnID) throws Exception {
        ...
        if (statusCode != HTTP_TEMPORARY_REDIRECT || response.getEntity() == null) {
            ...
            throw new DorisConnectorException(DorisConnectorErrorCode.STREAM_LOAD_FAILED, "Fail to abort transaction " + txnID + " with url " + abortUrlStr);
        }
        ...
        if (!SUCCESS.equals(res.get("status"))) {
            if (ResponseUtil.isCommitted(res.get("msg"))) {
                throw new DorisConnectorException(DorisConnectorErrorCode.STREAM_LOAD_FAILED, "try abort committed transaction, " +
