...
public class EdgeSocketPacketRecordDeserializer implements EdgeSocketRecordDeserializer {
    ...
    public EdgeSocketQueuedRecord deserializeRecord(String rawMessage) {
        try {
            ...
            EdgeSocketEncryptionType encryptionType =
                    EdgeSocketEncryptionType.from(defaultIfBlank(packet.getEncryption(), "NONE"));
            byte[] decryptedPayload = decodeEncryption(payloadBytes, packet, encryptionType);

            EdgeSocketCompressionType compressionType =
                    EdgeSocketCompressionType.from(defaultIfBlank(packet.getCompression(), "NONE"));
    ...
    private byte[] decodeEncryption(
        ...
        if (config.getAesSecretKeyBytes() == null || config.getAesSecretKeyBytes().length == 0) {
            throw new EdgeSocketConnectorException(
                    EdgeSocketConnectorErrorCode.PACKET_AES_KEY_MISSING,
                    "Missing aes_secret_key_base64 when packet encryption is AES_GCM");
        }
        ...
        try {
            ...
            SecretKeySpec key = new SecretKeySpec(config.getAesSecretKeyBytes(), "AES");
            cipher.init(
    ...
    /**
     * Normalize optional packet fields.
     *
     * @param value input value from packet field
     * @param defaultValue fallback when value is null/blank
     * @return normalized trimmed value
     */
