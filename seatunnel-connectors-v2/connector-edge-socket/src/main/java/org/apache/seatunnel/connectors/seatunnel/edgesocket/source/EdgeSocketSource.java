...
public class EdgeSocketSource extends AbstractSingleSplitSource<SeaTunnelRow> {
    ...
    public EdgeSocketSource(ReadonlyConfig pluginConfig) {
        this.parameter = new EdgeSocketConfig(pluginConfig);
        Optional<Map<String, Object>> schemaOptions =
                pluginConfig.getOptional(ConnectorCommonOptions.SCHEMA);
        if (schemaOptions.isPresent()) {
            // Use user-defined schema to deserialize JSON payload into typed SeaTunnelRow.
            this.catalogTable = CatalogTableUtil.buildWithConfig(pluginConfig);
            this.deserializationSchema = new JsonDeserializationSchema(catalogTable, false, false);
        } else {
            // Fallback schema keeps ingress simple: one string column named "value".
    ...
    public Boundedness getBoundedness() {
        // Edge socket ingress is long-running push mode by design.
    ...
    public AbstractSingleSplitReader<SeaTunnelRow> createReader(
        ...
        // Reader receives decoded payload and applies the selected row deserialization schema.
        return new EdgeSocketSourceReader(
                this.parameter, readerContext, this.deserializationSchema);
    }
}
