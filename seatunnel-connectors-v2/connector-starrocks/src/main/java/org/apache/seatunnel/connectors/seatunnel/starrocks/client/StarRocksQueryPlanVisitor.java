...
import org.apache.seatunnel.connectors.seatunnel.starrocks.source.model.QueryPlan;
...
@Slf4j
public class StarRocksQueryPlanVisitor {
    ...
    public StarRocksQueryPlanVisitor(SourceConfig sourceConfig, SeaTunnelRowType seaTunnelRowType) {
    ...
    private QueryPlan getQueryPlan(String querySQL, String httpNode) {
        ...
        try {
            ...
            throw new RuntimeException("Request of get query plan failed", e);
        }

        if (StringUtils.isEmpty(respString)) {
            throw new RuntimeException("Request of get query plan failed with empty response");
        }
