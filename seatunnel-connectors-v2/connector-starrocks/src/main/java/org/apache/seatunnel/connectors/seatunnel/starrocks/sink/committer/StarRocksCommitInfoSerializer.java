...
public class StarRocksCommitInfoSerializer implements Serializer<StarRocksCommitInfo> {
    ...
    @Override
    public byte[] serialize(StarRocksCommitInfo starRocksCommitInfo) throws IOException {
        try (final ByteArrayOutputStream baos = new ByteArrayOutputStream();
                final DataOutputStream out = new DataOutputStream(baos)) {
            out.writeUTF(starRocksCommitInfo.getDatabase());
            out.writeUTF(starRocksCommitInfo.getTable());
            out.writeUTF(starRocksCommitInfo.getLabel());
    ...
    public StarRocksCommitInfo deserialize(byte[] serialized) throws IOException {
        try (final ByteArrayInputStream bais = new ByteArrayInputStream(serialized);
            ...
            return new StarRocksCommitInfo(database, table, label);
        }
    }
}
