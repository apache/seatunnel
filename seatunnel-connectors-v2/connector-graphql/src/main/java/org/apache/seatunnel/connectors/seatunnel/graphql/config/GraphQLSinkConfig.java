...
public class GraphQLSinkConfig extends HttpConfig {

    public static final Option<String> QUERY =
            Options.key("query").stringType().noDefaultValue().withDescription("GraphQL query");

    public static final Option<String> OPERATION_NAME =
            Options.key("operationName")
                    .stringType()
                    .noDefaultValue()
                    .withDescription("GraphQL query");
    ...
    public static OptionRule getOptionRule() {
        return OptionRule.builder()
                .required(QUERY)
                .optional(TIMEOUT, OPERATION_NAME, VALUE_COVER, VARIABLES)
                .build();
    }
}
