...
public class ObsConf extends HadoopConf {
    ...
    public static HadoopConf buildWithConfig(ReadonlyConfig config) {
        HadoopConf hadoopConf = new ObsConf(config.get(ObsFileBaseOptions.BUCKET));
        HashMap<String, String> ossOptions = new HashMap<>();
        ossOptions.put(Constants.ACCESS_KEY, config.get(ObsFileBaseOptions.ACCESS_KEY));
        ossOptions.put(Constants.SECRET_KEY, config.get(ObsFileBaseOptions.ACCESS_SECRET));
        ossOptions.put(Constants.ENDPOINT, config.get(ObsFileBaseOptions.ENDPOINT));
