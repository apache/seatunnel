...
public class OssConf extends HadoopConf {
    ...
    public static HadoopConf buildWithConfig(ReadonlyConfig config) {
        HadoopConf hadoopConf = new OssConf(config.get(OssFileBaseOptions.BUCKET));
        ...
        ossOptions.put("fs.oss.accessKeyId", config.get(OssFileBaseOptions.ACCESS_KEY));
        ossOptions.put("fs.oss.accessKeySecret", config.get(OssFileBaseOptions.ACCESS_SECRET));
        ossOptions.put("fs.oss.endpoint", config.get(OssFileBaseOptions.ENDPOINT));
