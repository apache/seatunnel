...
public class CassandraSourceSplitEnumerator implements SourceSplitEnumerator<CassandraSourceSplit, CassandraSourceState> {

    private final Context<CassandraSourceSplit> context;
    private final Set<Integer> readers;
    private volatile int assigned = -1;

    CassandraSourceSplitEnumerator(Context<CassandraSourceSplit> context) {
    ...
    public void registerReader(int subtaskId) {
        ...
        if (assigned < 0) {
            assigned = subtaskId;
            context.assignSplit(subtaskId, new CassandraSourceSplit());
        }
    }

    @Override
    public CassandraSourceState snapshotState(long checkpointId) throws Exception {
        return null;
    }
