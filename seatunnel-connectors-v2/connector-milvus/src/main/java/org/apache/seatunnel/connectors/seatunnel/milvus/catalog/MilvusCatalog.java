...
public class MilvusCatalog implements Catalog {
        ...
        try {
            ...
            for (Column column : tableSchema.getColumns()) {
                if (column.getOptions() != null
                        && column.getOptions().containsKey(MilvusOptions.DYNAMIC_FIELD)
                        && (Boolean) column.getOptions().get(MilvusOptions.DYNAMIC_FIELD)) {
                    // skip dynamic field
                    continue;
                }
                fieldTypes.add(
                        convertToFieldType(column, tableSchema.getPrimaryKey(), partitionKeyField));
            }
    ...
    private FieldType convertToFieldType(
