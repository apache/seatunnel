...
import org.apache.seatunnel.connectors.seatunnel.tidb.sink.xa.GroupXaOperationResult;
...
public class TiDBSinkAggregatedCommitter
    ...
    public TiDBSinkAggregatedCommitter(
    ...
    public List<JdbcAggregatedCommitInfo> commit(List<JdbcAggregatedCommitInfo> aggregatedCommitInfos) throws IOException {
        ...
        return aggregatedCommitInfos.stream().map(aggregatedCommitInfo -> {
            GroupXaOperationResult<XidInfo> result = xaGroupOps.commit(aggregatedCommitInfo.getXidInfoList(), false, jdbcConnectionOptions.getMaxCommitAttempts());
