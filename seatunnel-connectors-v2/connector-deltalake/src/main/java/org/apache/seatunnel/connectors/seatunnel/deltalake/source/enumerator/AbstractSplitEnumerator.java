...
public abstract class AbstractSplitEnumerator
        implements SourceSplitEnumerator<DeltaLakeFileScanTaskSplit, DeltaLakeSplitEnumeratorState> {
    ...
    protected final Map<Integer, List<DeltaLakeFileScanTaskSplit>> pendingSplits;
    ...
    public AbstractSplitEnumerator(
        ...
        if (state == null) {
            ...
            state.getPendingSplits().values().stream()
                    ...
                    .map(
                            (Function<DeltaLakeFileScanTaskSplit, DeltaLakeFileScanTaskSplit>)
                                    split -> {
    ...
    @Override
    public void addSplitsBack(List<DeltaLakeFileScanTaskSplit> splits, int subtaskId) {
        if (!splits.isEmpty()) {
    ...
    protected Table loadTable(TablePath tablePath) {
        return new DeltaLakeCatalogLoader(sourceConfig).loadTable(tablePath);
    ...
    protected void addPendingSplits(Collection<DeltaLakeFileScanTaskSplit> newSplits) {
        int numReaders = context.currentParallelism();
        for (DeltaLakeFileScanTaskSplit newSplit : newSplits) {
    ...
    protected void assignPendingSplits(Set<Integer> pendingReaders) {
        for (int pendingReader : pendingReaders) {
            List<DeltaLakeFileScanTaskSplit> pendingAssignmentForReader =
