...
@Slf4j
public class DeltaLakeStreamSplitEnumerator extends AbstractSplitEnumerator {

  private final ConcurrentMap<TablePath, DeltaLakeEnumeratorPosition> tableOffsets;
  private volatile boolean initialized = false;

  public DeltaLakeStreamSplitEnumerator(
  ...
  public DeltaLakeStreamSplitEnumerator(
    ...
    super(context, sourceConfig, catalogTables, engine, state);
    ...
    while (true) {
      for (TablePath tablePath : pendingTables) {
        ...
        synchronized (stateLock) {
          ...
          Collection<DeltaLakeFileScanTaskSplit> splits = loadSplits(tablePath);
  ...
  @Override
  public DeltaLakeSplitEnumeratorState snapshotState(long checkpointId) throws Exception {
    synchronized (stateLock) {
  ...
  private List<DeltaLakeFileScanTaskSplit> loadSplits(TablePath tablePath) {
    ...
    DeltaLakeEnumeratorPosition offset = tableOffsets.get(tablePath);
    ...
    DeltaLakeEnumerationResult result =
