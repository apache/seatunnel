 ...
 * </pre>
 *
 * Certain data types are not emitted as string literals, such as {@code DATE}/{@code TIME} and {@code TIMESTAMP}/{@code DATETIME}..
 ...
 * </pre>
 *
 * The new value for {@code C1} would be {@code TIMESTAMP'2020-02-02 00:00:00'}.
 * The old value for {@code C1} would be {@code TIMESTAMP'2020-02-01 00:00:00'}.
 *
 */
...
public class LogMinerDmlParser implements DmlParser {
    ...
    @Override
    public LogMinerDmlEntry parse(@NonNull String sql,
                                  @NonNull Table table,
                                  String txId) {
    ...
    @Override
    public LogMinerDmlEntry parseInsert(@NonNull String sql,
                                        @NonNull Table table,
                                        String txId) {
        SQLReader reader = new SQLReader(sql);
        checkArgument(reader.equalsIgnoreCaseAndMove(INSERT)
        ...
        checkArgument(reader.equalsIgnoreCaseAndMove("values")
                || reader.equalsIgnoreCaseAndMove("value"),
            "Can't found 'values' or 'value'");
        checkArgument(reader.current('(')
                || reader.nextAndSkip(SKIP_CHECKER),
            "Can't found 'values' or 'value'");
    ...
    @Override
    public LogMinerDmlEntry parseUpdate(@NonNull String sql,
                                        @NonNull Table table,
                                        String txId) {
        SQLReader reader = new SQLReader(sql + SQL_END_OF);
        checkArgument(reader.equalsIgnoreCaseAndMove(UPDATE)
                && reader.nextAndSkip(SKIP_CHECKER),
            "SQL must start with 'UPDATE'");

        TableId tableId = readTableId(reader);
        checkArgument(reader.equalsIgnoreCaseAndMove("set")
                && reader.nextAndSkip(SKIP_CHECKER),
    ...
    @Override
    public LogMinerDmlEntry parseDelete(@NonNull String sql,
                                        @NonNull Table table,
                                        String txId) {
        SQLReader reader = new SQLReader(sql + SQL_END_OF);
        checkArgument(reader.equalsIgnoreCaseAndMove(DELETE)
                && reader.nextAndSkip(SKIP_CHECKER)
    ...
    private static Map<String, Object> readWhereConditions(SQLReader reader) {
        checkArgument(reader.equalsIgnoreCaseAndMove("where"),
            "Can't found 'where'");

        Map<String, Object> oldColumnValues = new HashMap<>();
        for (int i = 0; reader.nextAndSkip(SKIP_CHECKER) && !reader.current(SQL_END_OF); i++) {
            if (i != 0) {
                checkArgument(reader.equalsIgnoreCaseAndMove("and"),
                    "Condition not start 'and'");
                reader.nextAndSkip(SKIP_CHECKER);
            }
            String columnName = readName(reader, "Can't found column name");
            reader.nextAndSkip(SKIP_CHECKER);
