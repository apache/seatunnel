 ...
 * MySqlDebeziumAdapter} reports, and the Debezium actually resolved onto the connector's classpath.
 * If they diverge, a job would run on a Debezium build that nothing in the project claims to
 * support.
 */
class MySqlDebeziumAdapterTest {
    ...
    void declaredVersionMatchesPackagedDebezium() throws Exception {
        Assertions.assertEquals(
                resolvedDebeziumCoreVersion(), new MySqlDebeziumAdapter().getDebeziumVersion());
    }

    private static String resolvedDebeziumCoreVersion() throws Exception {
        try (InputStream in =
                MySqlDebeziumAdapterTest.class
                        .getClassLoader()
