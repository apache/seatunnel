...
public class MySqlStreamingChangeEventSource
    ...
    public void execute(ChangeEventSourceContext context, MySqlOffsetContext offsetContext)
        ...
        if (connectorConfig.bufferSizeForStreamingChangeEventSource() == 0) {
            eventListener = (event) -> handleEvent(effectiveOffsetContext, event);
        } else {
            EventBuffer buffer =
                    new EventBuffer(
                            ...
                            this,
                            context);
            eventListener = (event) -> buffer.add(effectiveOffsetContext, event);
        }
    ...
    protected void releaseResource() {
        // Unregister event listener and close connection
        try {
            client.unregisterEventListener(eventListener);
            connection.close();
        } catch (Exception e) {
            LOGGER.info(
                    "An exception occurred when unregister event listener or closing the connection after the binlog split read",
                    e);
        }
    }
}
