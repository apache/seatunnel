 ...
 * of the unprocessed transaction and the LSN from which the streaming should start. Backports
 * DBZ-5915 and DBZ-6204 to the bundled Debezium 1.9.8 implementation.
 *
 * <p>Copyright Debezium Authors. Adapted from Debezium 1.9.8.Final.
 *
...
public class WalPositionLocator {
    ...
    public WalPositionLocator(Lsn lastCommitStoredLsn, Lsn lastEventStoredLsn) {
        this(lastCommitStoredLsn, lastEventStoredLsn, null);
    }
        ...
        if (storeLsnAfterLastEventStoredLsn) {
            ...
            if (currentLsn.equals(lastEventStoredLsn)) {
                ...
                if (txStartLsn != null
                    ...
                    // The previous COMMIT can share its LSN with this BEGIN and INSERT.
                    // Replay this transaction unless the stored operation was already a row.
