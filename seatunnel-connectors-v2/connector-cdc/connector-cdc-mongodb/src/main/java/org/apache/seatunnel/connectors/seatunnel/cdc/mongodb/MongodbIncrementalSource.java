...
public class MongodbIncrementalSource<T> extends IncrementalSource<T, MongodbSourceConfig>
    ...
    public SourceConfig.Factory<MongodbSourceConfig> createSourceConfigFactory(
        ...
        Optional.ofNullable(config.get(MongodbSourceOptions.FULL_INCREMENTAL_INTEGRATION))
                .ifPresent(builder::fullIncrementalIntegration);
        Optional.ofNullable(config.get(MongodbSourceOptions.FULL_INCREMENTAL_QUEUE_SIZE))
                .ifPresent(builder::fullIncrementalQueueSize);
