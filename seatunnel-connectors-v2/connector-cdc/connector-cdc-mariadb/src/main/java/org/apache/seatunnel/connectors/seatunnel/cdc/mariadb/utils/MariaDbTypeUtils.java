...
/** Utilities for converting from MariaDB types to SeaTunnel types. */
@Slf4j
public class MariaDbTypeUtils {

    private static final MariaDbTypeConverter NO_INT_NARROWING_CONVERTER =
            new MariaDbTypeConverter(false);
    ...
    public static org.apache.seatunnel.api.table.catalog.Column convertToSeaTunnelColumn(
        ...
        switch (column.typeName().toUpperCase()) {
            case MariaDbTypeConverter.MARIADB_CHAR:
            case MariaDbTypeConverter.MARIADB_VARCHAR:
                if (column.length() <= 0) {
                    builder.columnType(MariaDbTypeConverter.MARIADB_VARCHAR);
                    builder.length(TypeDefineUtils.charTo4ByteLength(1L));
                } else {
                    builder.columnType(
                            String.format(
                                    "%s(%s)",
                                    MariaDbTypeConverter.MARIADB_VARCHAR, column.length()));
                    builder.length(TypeDefineUtils.charTo4ByteLength((long) column.length()));
                }
                break;
            case MariaDbTypeConverter.MARIADB_TIME:
                if (column.length() <= 0) {
                    builder.columnType(MariaDbTypeConverter.MARIADB_TIME);
                } else {
                    builder.columnType(
                            String.format(
                                    "%s(%s)", MariaDbTypeConverter.MARIADB_TIME, column.length()));
                    builder.scale(column.length());
                }
                break;
            case MariaDbTypeConverter.MARIADB_TIMESTAMP:
                if (column.length() <= 0) {
                    builder.columnType(MariaDbTypeConverter.MARIADB_TIMESTAMP);
                } else {
                    builder.columnType(
                            String.format(
                                    "%s(%s)",
                                    MariaDbTypeConverter.MARIADB_TIMESTAMP, column.length()));
                    builder.scale(column.length());
                }
                break;
            case MariaDbTypeConverter.MARIADB_DATETIME:
                if (column.length() <= 0) {
                    builder.columnType(MariaDbTypeConverter.MARIADB_DATETIME);
                } else {
                    builder.columnType(
                            String.format(
                                    "%s(%s)",
                                    MariaDbTypeConverter.MARIADB_DATETIME, column.length()));
                    builder.scale(column.length());
                }
                break;
            case MariaDbTypeConverter.MARIADB_TINYINT:
                if (column.length() > 0) {
        ...
        return (intTypeNarrowing
                        ? MariaDbTypeConverter.DEFAULT_INSTANCE
