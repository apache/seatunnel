...
public class MariaDbSnapshotFetchTask implements FetchTask<SourceSplitBase> {
    private static final Logger log = LoggerFactory.getLogger(MariaDbSnapshotFetchTask.class);

    private final SnapshotSplit split;

    private volatile boolean taskRunning = false;

    private MariaDbSnapshotSplitReadTask snapshotSplitReadTask;

    public MariaDbSnapshotFetchTask(SnapshotSplit split) {
        this.split = split;
    }
    ...
    public void execute(FetchTask.Context context) throws Exception {
        ...
        snapshotSplitReadTask =
                new MariaDbSnapshotSplitReadTask(
        ...
        // optimization that skip the binlog read when the low watermark equals high watermark
        if (!changed) {
        ...
        final MariaDbBinlogFetchTask.MariaDbBinlogSplitReadTask backfillReadTask =
    ...
    private MariaDbBinlogFetchTask.MariaDbBinlogSplitReadTask createBackfillBinlogReadTask(
            IncrementalSplit backfillBinlogSplit, MariaDbSourceFetchTaskContext context) {
        ...
        Configuration dezConf =
                context.getSourceConfig()
                        .getDbzConfiguration()
                        .edit()
                        .with(MariaDbSourceConfigFactory.SCHEMA_CHANGE_KEY, "false")
                        .with("table.include.list", split.getTableId().toString())
                        // Disable heartbeat event in snapshot split fetcher
                        .with(Heartbeat.HEARTBEAT_INTERVAL, 0)
                        .build();
        return new MariaDbBinlogFetchTask.MariaDbBinlogSplitReadTask(
