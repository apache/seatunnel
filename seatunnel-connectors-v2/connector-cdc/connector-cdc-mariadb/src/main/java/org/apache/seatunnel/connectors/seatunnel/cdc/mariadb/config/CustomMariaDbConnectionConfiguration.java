...
public class CustomMariaDbConnectionConfiguration
        extends MySqlConnection.MySqlConnectionConfiguration {

    protected static final String URL_PATTERN =
            "jdbc:mariadb://${hostname}:${port}/?useInformationSchema=true&connectTimeout=${connectTimeout}";

    private final JdbcConnection.ConnectionFactory connectionFactory;

    public CustomMariaDbConnectionConfiguration(Configuration config) {
        super(config);
        String driverClassName = config.getString(JDBC_DRIVER.name(), "org.mariadb.jdbc.Driver");
        connectionFactory =
