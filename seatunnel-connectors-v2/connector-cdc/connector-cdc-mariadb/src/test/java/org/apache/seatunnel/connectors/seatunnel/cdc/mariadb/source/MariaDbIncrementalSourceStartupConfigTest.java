...
public class MariaDbIncrementalSourceStartupConfigTest {
    ...
    public void testOptionRuleAcceptsGtidSpecificStartup() {
        ...
        options.put(SourceOptions.STARTUP_SPECIFIC_OFFSET_FILE.key(), "mariadb-bin.000123");
        options.put(SourceOptions.STARTUP_SPECIFIC_OFFSET_POS.key(), 456789L);
        options.put(
                MariaDbIncrementalSourceOptions.STARTUP_SPECIFIC_OFFSET_GTID_SET.key(), GTID_SET);

        ConfigValidator.of(ReadonlyConfig.fromMap(options))
                .validate(new MariaDbIncrementalSourceFactory().optionRule());
    }

    @Test
    public void testCreateStartupConfigWithGtidSetAndSkipFields() {
        StartupConfig startupConfig =
                MariaDbIncrementalSource.createStartupConfig(
                        config(
        ...
        Assertions.assertEquals(
                "mariadb-bin.000123",
                startupOffset.getOffset().get(MariaDbBinlogOffset.BINLOG_FILENAME_OFFSET_KEY));
        Assertions.assertEquals(
                "456789",
                startupOffset.getOffset().get(MariaDbBinlogOffset.BINLOG_POSITION_OFFSET_KEY));
        Assertions.assertEquals(
                GTID_SET, startupOffset.getOffset().get(MariaDbBinlogOffset.GTID_SET_KEY));
        Assertions.assertEquals(
                "3", startupOffset.getOffset().get(MariaDbBinlogOffset.EVENTS_TO_SKIP_OFFSET_KEY));
        Assertions.assertEquals(
                "10", startupOffset.getOffset().get(MariaDbBinlogOffset.ROWS_TO_SKIP_OFFSET_KEY));
    }
    ...
    public void testCreateStartupConfigWithFilePositionAndSkipFields() {
        ...
        Assertions.assertEquals(
                ...
                startupOffset.getOffset().get(MariaDbBinlogOffset.BINLOG_POSITION_OFFSET_KEY));
        Assertions.assertEquals(
                "0", startupOffset.getOffset().get(MariaDbBinlogOffset.EVENTS_TO_SKIP_OFFSET_KEY));
        Assertions.assertEquals(
                "2", startupOffset.getOffset().get(MariaDbBinlogOffset.ROWS_TO_SKIP_OFFSET_KEY));
    }
    ...
    public void testSpecificStartupOffsetLoadsThroughDebeziumOffsetLoader() {
        ...
        Assertions.assertEquals("mariadb-bin.000123", offsetContext.getSource().binlogFilename());
    ...
    private static Map<String, Object> requiredOptions() {
        Map<String, Object> options = new HashMap<>();
        options.put(MariaDbIncrementalSourceOptions.USERNAME.key(), "mariadbuser");
        options.put(MariaDbIncrementalSourceOptions.PASSWORD.key(), "mariadbpw");
        options.put(
                MariaDbIncrementalSourceOptions.URL.key(), "jdbc:mariadb://localhost:3306/test");
        options.put(
                MariaDbIncrementalSourceOptions.TABLE_NAMES.key(),
    ...
    private static class TestOffsetFactory extends OffsetFactory {
        ...
        public Offset specific(String filename, Long position) {
            ...
            offset.put(MariaDbBinlogOffset.BINLOG_FILENAME_OFFSET_KEY, filename);
            offset.put(MariaDbBinlogOffset.BINLOG_POSITION_OFFSET_KEY, String.valueOf(position));
            return new TestOffset(offset);
        }
