...
package org.apache.seatunnel.connectors.seatunnel.cdc.db2.source.offset;
...
public class LsnOffset extends Offset {
    ...
    public static LsnOffset valueOf(String commitLsn) {
        return new LsnOffset(Lsn.valueOf(commitLsn), null);
    }

    public static LsnOffset valueOf(String commitLsn, String changeLsn) {
        return new LsnOffset(Lsn.valueOf(commitLsn), Lsn.valueOf(changeLsn));
    }
    ...
    private LsnOffset(Lsn commitLsn, Lsn changeLsn) {
    ...
    public int compareTo(Offset o) {
        ...
        // Snapshot high-watermarks only carry a commit LSN. In that case commit LSN ordering is
        // the boundary contract and row-level in-transaction LSN must not keep the bounded reader
        // running forever on events from the same commit.
        if (comparison == 0
                && (!getChangeLsn().isAvailable() || !that.getChangeLsn().isAvailable())) {
            return 0;
        }
