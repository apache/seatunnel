...
public class OscarCreateTableSqlBuilder extends AbstractJdbcCreateTableSqlBuilder {
    ...
    public OscarCreateTableSqlBuilder(CatalogTable catalogTable, boolean createIndex) {
    ...
    String buildColumnSql(Column column) {
        ...
        } else if (StringUtils.equals(DatabaseIdentifier.OSCAR, sourceCatalogName)
                && StringUtils.isNotEmpty(column.getSourceType())) {
            columnType = column.getSourceType();
        } else {
            columnType = OscarTypeConverter.INSTANCE.reconvert(column).getColumnType();
        }
