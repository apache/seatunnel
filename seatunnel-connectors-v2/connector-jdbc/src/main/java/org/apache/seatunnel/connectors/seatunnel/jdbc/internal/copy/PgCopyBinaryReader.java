...
public final class PgCopyBinaryReader implements PgCopyReader {
    ...
    private final int maxBufferSize; // upper bound to prevent unbounded growth
    ...
    public PgCopyBinaryReader(InputStream stream, TableSchema schema, Integer pgCopyBufferSize) {
        ...
        this.maxBufferSize = Math.min(this.bufferSize * 1024, MAX_ALLOWED_BUFFER_SIZE);
        this.buffer = ByteBuffer.allocate(this.bufferSize).order(ByteOrder.BIG_ENDIAN);
    }
    ...
    private void ensureCapacityFor(int required) {
        ...
        while (newCap < required && newCap < maxBufferSize) newCap = newCap << 1;
        if (newCap < required) {
            throw new JdbcConnectorException(
                    CommonErrorCodeDeprecated.UNSUPPORTED_OPERATION,
