...
@Slf4j
public class DateTimePartitionSplit extends AbstractPartitionSplit<Timestamp> {

    private final JdbcConnectionProvider jdbcConnectionProvider;

    public DateTimePartitionSplit(JdbcConnectionProvider jdbcConnectionProvider, JdbcSourceOptions jdbcSourceOptions, SeaTunnelRowType rowType) {
    ...
    public boolean checkType(SeaTunnelDataType<?> type) {
        return type.equals(LocalTimeType.LOCAL_TIME_TYPE) ||
            type.equals(LocalTimeType.LOCAL_DATE_TIME_TYPE);
    }

    @Override
    public PartitionParameter<Timestamp> getPartitionParameter(Map<String, SeaTunnelDataType<?>> fieldTypes, String partitionColumn) throws SQLException {
        if (!checkType(fieldTypes.get(partitionColumn))) {
            throw new JdbcConnectorException(CommonErrorCode.ILLEGAL_ARGUMENT,
                String.format("%s is not time type", partitionColumn));
        }
        ...
        if (jdbcSourceOptions.getPartitionLowerBound().isPresent() &&
            ...
            return new PartitionParameter<>(partitionColumn, new Timestamp(min), new Timestamp(max), jdbcSourceOptions.getPartitionNumber().orElse(null));
        }
    ...
    public Set<JdbcSourceSplit> getSplit(int currentParallelism) throws SQLException {
        ...
        PartitionParameter<Timestamp> partitionParameter = checkAndGetPartitionColumn();
        if (null != partitionParameter) {
            ...
            JdbcParameterValuesProvider jdbcNumericBetweenParametersProvider =
                new JdbcDateTimeBetweenParametersProvider(partitionParameter.getMinValue().getTime(), partitionParameter.getMaxValue().getTime())
