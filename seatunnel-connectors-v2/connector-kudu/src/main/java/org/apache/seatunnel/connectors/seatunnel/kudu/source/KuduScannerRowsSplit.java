...
public class KuduScannerRowsSplit {
    ...
    public KuduScannerRowsSplit(int maxBucketNum, int bucketCapacity, Type keyType) {
        ...
        scannerRowsSplitBucket =
                new ArrayList<>((maxBucketNum >> 2 > 0 ? maxBucketNum >> 2 : 1) + 1);
    }
    ...
    public void add(Object intervalValue) {
        ...
        for (; i < scannerRowsSplitBucket.size(); i++) {
            compare =
                    KuduColumn.KeyColCompare(intervalValue, scannerRowsSplitBucket.get(i), keyType);
            if (compare <= 0) {
                break;
            }
        }
    ...
    public void removeLesser(Object start) {
        ...
        if (index >= getSize()) {
            ...
            if (index != 0
                    && KuduColumn.KeyColCompare(start, scannerRowsSplitBucket.get(index), keyType)
                            != 0) {
                /* Set lower bound */
    ...
    public KuduScannerRowsSplit getLesser(Object end) {
        KuduScannerRowsSplit newScannerRowsSplit =
                new KuduScannerRowsSplit(maxBucketNum, bucketCapacity, keyType);
