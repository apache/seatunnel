...
public class KuduColumn {
    public static int KeyColCompare(Object colVal1, Object colVal2, Type colType) {
        switch (colType) {
            ...
            case INT16:
            case INT32:
            case INT64: return (int) colVal1 - (int) colVal2;
            case UNIXTIME_MICROS: return ((Timestamp) colVal1).compareTo((Timestamp) colVal2);
            case STRING: return (String.valueOf(colVal1)).compareTo(String.valueOf(colVal2));
            case DECIMAL: return ((BigDecimal) colVal1).compareTo((BigDecimal) colVal2);
            case BINARY: return compareBinary((byte[]) colVal1, (byte[]) colVal2);
            default:
    ...
    public static Serializable[] getSerializable(Type colType, Object... colVals) {
        ...
        for (Object value : colVals) {
            switch (colType) {
                ...
                case INT16:
                case INT32:
                case INT64: serializes[i] = (Long) value; break;
                case UNIXTIME_MICROS: serializes[i] = (Timestamp) value; break;
                case STRING: serializes[i] = String.valueOf(value); break;
                case DECIMAL: serializes[i] = (BigDecimal) value; break;
                case BINARY: serializes[i] = (byte[]) value; break;
                default:
                    throw new UnsupportedOperationException("Unsupported type: " + colType);
            }
            i++;
    ...
    public static Object addValue(Type colType, Object colVal) {
        switch (colType) {
            ...
            case INT16:
            case INT32:
            case INT64: return ((Long) colVal) + 1;
            case UNIXTIME_MICROS:
                LocalDateTime localDateTime = ((Timestamp) colVal).toLocalDateTime().plusSeconds(1);
                return Timestamp.valueOf(localDateTime);
            case STRING: return String.valueOf(colVal) + '1';
            case DECIMAL: return ((BigDecimal) colVal).add(new BigDecimal(1));
            case BINARY:
