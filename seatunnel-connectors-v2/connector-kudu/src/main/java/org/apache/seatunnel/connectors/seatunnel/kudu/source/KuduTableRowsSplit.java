...
public class KuduTableRowsSplit {
    ...
    public void add(KuduScannerRowsSplit rowsSplit) {
        ...
        int index = 0;
        boolean isEnd = false;
        LinkedList<KuduScannerRowsSplit> tmpKuduScannerRowsSplit =
                new LinkedList<>(kuduTableRowsSplit);
        ...
        if (bucketNum >= maxBucketNum || maxBucketCapacity != rowsSplit.getBucketCapacity()) {
            ...
            if (tmpMaxBucketCapacity <= rowsSplit.getBucketCapacity()) {
                ...
                for (KuduScannerRowsSplit kuduScannerRowsSplit : kuduTableRowsSplit) {
                    boolean isSuccess =
                            extendScannerBucketCapacity(kuduScannerRowsSplit, tmpMaxBucketCapacity);
                    if (isSuccess) {
    ...
    private boolean extendScannerBucketCapacity(
            KuduScannerRowsSplit kuduScannerRowsSplit, int extendBucketCapacity) {
        ...
        for (KuduScannerRowsSplit kuduScannerRowsSplit : kuduTableRowsSplit) {
            ...
            for (int i = 1; i < kuduScannerRowsSplit.getSize(); i++) {
                if ((i == 1 && kuduScannerRowsSplit.isFirstBucketSplit())
                        || (i == kuduScannerRowsSplit.getBucketNum()
                                && kuduScannerRowsSplit.isLastBucketSplit())) {
