...
public class SelectDBSinkFactory implements TableSinkFactory {
    ...
    public OptionRule optionRule() {
        return OptionRule.builder()
                .required(
                        ...
                        SelectDBSinkOptions.DATA_SAVE_MODE,
                        SelectDBSinkOptions.SCHEMA_SAVE_MODE)
                .optional(
                        SelectDBSinkOptions.DATABASE,
                        SelectDBSinkOptions.TABLE,
                        SelectDBSinkOptions.TABLE_IDENTIFIER,
                        SelectDBSinkOptions.QUERY_PORT,
                        SelectDBSinkOptions.SELECTDB_BATCH_SIZE,
                        SelectDBSinkOptions.SINK_ENABLE_2PC,
                        SelectDBSinkOptions.SINK_ENABLE_DELETE,
                        SelectDBSinkOptions.SAVE_MODE_CREATE_TEMPLATE,
                        SelectDBSinkOptions.NEEDS_UNSUPPORTED_TYPE_CASTING,
                        SelectDBSinkOptions.SINK_CHECK_INTERVAL,
                        SelectDBSinkOptions.SINK_MAX_RETRIES,
                        SelectDBSinkOptions.SINK_BUFFER_SIZE,
                        SelectDBSinkOptions.SINK_BUFFER_COUNT,
                        SelectDBSinkOptions.DEFAULT_DATABASE,
                        SinkConnectorCommonOptions.MULTI_TABLE_SINK_REPLICA)
                .conditional(
                        SelectDBSinkOptions.DATA_SAVE_MODE,
                        DataSaveMode.CUSTOM_PROCESSING,
                        SelectDBSinkOptions.CUSTOM_SQL)
                .build();
    }

    @Override
    public List<String> excludeTablePlaceholderReplaceKeys() {
        return Arrays.asList(SelectDBSinkOptions.SAVE_MODE_CREATE_TEMPLATE.key());
    }
    ...
    public TableSink<SeaTunnelRow, SelectDBSinkState, SelectDBCommitInfo, SelectDBCommitInfo> createSink(
        ...
        return () -> new SelectDBSink(config, finalCatalogTable);
    }
    ...
    private CatalogTable renameCatalogTable(ReadonlyConfig options, CatalogTable catalogTable) {
        ...
        String tableName;
        String databaseName;
        String tableIdentifier = options.get(SelectDBSinkOptions.TABLE_IDENTIFIER);
