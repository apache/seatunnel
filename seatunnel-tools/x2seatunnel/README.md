...
#### Build from Source
```bash
# Enter SeaTunnel project directory
cd /path/to/seatunnel

# build only x2seatunnel module
mvn clean package -pl seatunnel-tools/x2seatunnel -DskipTests
```
After compilation, you can get the ready-to-use release package seatunnel-tools/x2seatunnel/target/x2seatunnel-*.zip.
