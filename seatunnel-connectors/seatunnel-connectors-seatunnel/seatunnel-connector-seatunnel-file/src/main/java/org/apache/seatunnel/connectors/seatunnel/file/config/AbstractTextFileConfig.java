...
public class AbstractTextFileConfig implements DelimiterConfig, CompressConfig, Serializable {
    ...
    public AbstractTextFileConfig(@NonNull Config config) {
        ...
        if (!StringUtils.isBlank(config.getString(Constant.COMPRESS_CODEC))) {
            this.compressCodec = config.getString(Constant.COMPRESS_CODEC);
            throw new RuntimeException("compress not support now");
        }

        if (!StringUtils.isBlank(config.getString(Constant.FIELD_DELIMITER))) {
            this.fieldDelimiter = config.getString(Constant.FIELD_DELIMITER);
        }

        if (!StringUtils.isBlank(config.getString(Constant.ROW_DELIMITER))) {
            this.rowDelimiter = config.getString(Constant.ROW_DELIMITER);
        }

        if (!StringUtils.isBlank(config.getString(Constant.PATH))) {
            this.path = config.getString(Constant.PATH);
        }

        if (!StringUtils.isBlank(config.getString(Constant.FILE_NAME_EXPRESSION))) {
            this.fileNameExpression = config.getString(Constant.FILE_NAME_EXPRESSION);
        }

        if (!StringUtils.isBlank(config.getString(Constant.FILE_FORMAT))) {
