...
public class Elasticsearch implements FlinkStreamSink, FlinkBatchSink {
    ...
    public DataStreamSink<Row> outputStream(FlinkEnvironment env, DataStream<Row> dataStream) {
        ...
        ElasticsearchSink.Builder<Row> esSinkBuilder = new ElasticsearchSink.Builder<>(httpHosts,
                (ElasticsearchSinkFunction<Row>) (element, ctx, indexer) -> indexer.add(createIndexRequest(element, fieldNames))
        );
    ...
    private IndexRequest createIndexRequest(Row element, String[] fieldNames) {
        ...
        return Requests.indexRequest().index(indexName).type(config.getString(INDEX_TYPE))
                .source(json);
    }
}
