...
import org.apache.seatunnel.config.ConfigIncluderClasspath;
import org.apache.seatunnel.config.ConfigIncluderFile;
import org.apache.seatunnel.config.ConfigIncluderURL;
...
class SimpleIncluder implements FullIncluder {
    ...
    public ConfigIncluder withFallback(ConfigIncluder fallback) {
        if (this == fallback) {
            throw new BugOrBroken("trying to create includer cycle");
    ...
    static ConfigObject fromBasename(NameSource source, String name, ConfigParseOptions options) {
        ...
        if (name.endsWith(".conf") || name.endsWith(".json") || name.endsWith(".properties")) {
            ...
            List<IO> fails = new ArrayList<IO>();
            ...
            if (!options.getAllowMissing() && !gotSomething) {
                ...
                if (fails.isEmpty()) {
                    ...
                    throw new IO(SimpleConfigOrigin.newSimple(name), sb.toString(),
                            fails.get(0));
                }
