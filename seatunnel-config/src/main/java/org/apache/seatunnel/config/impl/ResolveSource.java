...
final class ResolveSource {
    ...
    private static ResultWithPath findInObject(AbstractConfigObject obj, ResolveContext context, Path path)
        ...
        throw new BugOrBroken("resolved object to non-object " + obj + " to " + partiallyResolved);
    }
    ...
    ResolveSource pushParent(Container parent) {
        if (parent == null) {
            throw new BugOrBroken("can't push null parent");
        }
    ...
    private static Node<Container> replace(Node<Container> list, Container old, AbstractConfigValue replacement) {
        ...
        if (child != old) {
            throw new BugOrBroken("Can only replace() the top node we're resolving; had " + child + " on top and tried to replace " + old + " overall list was " + list);
        }
    ...
    ResolveSource replaceCurrentParent(Container old, Container replacement) {
        ...
        throw new BugOrBroken("attempt to replace root " + root + " with " + replacement);
    }
