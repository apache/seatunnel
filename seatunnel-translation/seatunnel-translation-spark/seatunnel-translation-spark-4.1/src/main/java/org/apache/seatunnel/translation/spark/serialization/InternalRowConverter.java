...
public final class InternalRowConverter extends RowConverter<InternalRow> {
    ...
    private static Map<Object, Object> reconvertMap(
            scala.collection.immutable.Map<?, ?> immutableMap, MapType<?, ?> mapType) {
        if (immutableMap == null || immutableMap.isEmpty()) {
            return Collections.emptyMap();
        }
        Map<Object, Object> newMap = new LinkedHashMap<>(immutableMap.size());
        SeaTunnelDataType<?> keyType = mapType.getKeyType();
        SeaTunnelDataType<?> valueType = mapType.getValueType();
        scala.collection.Iterator<?> iterator = immutableMap.iterator();
        while (iterator.hasNext()) {
            Tuple2<?, ?> entry = (Tuple2<?, ?>) iterator.next();
            Object key = reconvert(entry._1(), keyType);
            Object value = reconvert(entry._2(), valueType);
    ...
    private static Object reconvert(Object field, SeaTunnelDataType<?> dataType) {
        ...
        switch (dataType.getSqlType()) {
            ...
            case MAP:
                ...
                } else if (field instanceof scala.collection.immutable.Map) {
                    return reconvertMap(
                            (scala.collection.immutable.Map<?, ?>) field, (MapType<?, ?>) dataType);
                } else {
            ...
            case ARRAY:
                if (field instanceof ArrayData) {
                    ...
                    return reconvertObjectArray((Object[]) field, (ArrayType<?, ?>) dataType);
                } else if (field instanceof scala.collection.IndexedSeq) {
                    return reconvertObjectArray(
                            indexedSeqToArray((scala.collection.IndexedSeq<?>) field),
                            (ArrayType<?, ?>) dataType);
                } else {
                    throw new RuntimeException(
                            String.format(
    ...
    private static Object reconvertObjectArray(Object[] arrayData, ArrayType<?, ?> arrayType) {
        if (arrayData == null || arrayData.length == 0) {
            return Collections.emptyList().toArray();
        }
        Object[] newArray = new Object[arrayData.length];
        for (int i = 0; i < arrayData.length; i++) {
            newArray[i] = reconvert(arrayData[i], arrayType.getElementType());
        }
        return newArray;
    }

    private static Object[] indexedSeqToArray(scala.collection.IndexedSeq<?> seq) {
        Object[] array = new Object[seq.size()];
    ...
    private Object convertToField(Object internalRowField, DataType dataType) {
        if (dataType == DataTypes.TimestampType && internalRowField instanceof Long) {
            ...
            scala.collection.immutable.Map<Object, Object> newMap =
