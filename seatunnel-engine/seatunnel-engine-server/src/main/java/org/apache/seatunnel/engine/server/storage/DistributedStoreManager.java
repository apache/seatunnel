...
public class DistributedStoreManager {
    ...
    public static <K, V> StateStore<K, V> getMap(NodeEngineImpl nodeEngine, String mapName) {
        return new HazelcastStateStoreFactory(nodeEngine).getMap(mapName);
    }

    public <K, V> StateStore<K, V> getMap(String mapName) {
        return stateStoreFactory.getMap(mapName);
    }
}
