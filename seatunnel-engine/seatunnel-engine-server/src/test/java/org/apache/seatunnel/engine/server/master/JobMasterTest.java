...
class JobMasterTest extends AbstractSeaTunnelServerTest {
    /**
     * StateStore key is jobId and value is a Tuple2 Tuple2 key is JobMaster init timestamp and
     * value is the jobImmutableInformation which is sent by client when submit job
     *
     * <p>This StateStore is used to recovery runningJobInfoStateStore in JobMaster when a new
     * master node active
     */
     ...
     *
     * <p>This StateStore is used to recovery runningJobStateTimestampsStateStore in JobMaster when
     * a new master node active
     */
    StateStore<Object, Long[]> runningJobStateTimestampsStore;
     ...
     * used.
     *
     * <p>This StateStore is used to recovery ownedSlotProfilesStateStore in JobMaster when a new
     * master node active
     */
    ...
    void testHandleCheckpointTimeout() throws Exception {
        ...
        testStateStoreRemovedAfterJobComplete(jobId, jobMaster);
    }

    private void testStateStoreRemovedAfterJobComplete(long jobId, JobMaster jobMaster) {
        ...
        await().atMost(60000, TimeUnit.MILLISECONDS)
                .untilAsserted(
                        () -> {
                            Assertions.assertNull(runningJobInfoStore.get(jobId));
                            Assertions.assertNull(runningJobStateStore.get(jobId));
                            Assertions.assertNull(runningJobStateTimestampsStore.get(jobId));
                            Assertions.assertNull(ownedSlotProfilesStore.get(jobId));
                                    ...
                                    .forEach(
                                            pipeline -> {
                                                Assertions.assertNull(
                                                        runningJobStateStore.get(
                                                                pipeline.getPipelineLocation()));

                                                Assertions.assertNull(
                                                        runningJobStateTimestampsStore.get(
                                                                pipeline.getPipelineLocation()));
                                            });
                            jobMaster
                                    .getPhysicalPlan()
