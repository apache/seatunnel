...
class ManagedSourceRuntimeConfigTest {
    ...
    @Test
    void shouldParseManagedSourceRuntimeLimits() throws Exception {
        String configPath =
                Paths.get(
                                ManagedSourceRuntimeConfigTest.class
                                        .getResource("/managed-source-runtime.yaml")
                                        .toURI())
                        .toString();
        ...
        Assertions.assertEquals(16_777_216L, config.getAssignmentTrackerMaxBytes());
    }

    @Test
    void shouldReserveOneCompletionSlotPerAsyncWorker() {
        ManagedSourceRuntimeConfig config = new ManagedSourceRuntimeConfig();
        config.setReaderReservedControlCommands(3);
        config.setCoordinatorAsyncMaxConcurrency(4);

        IllegalArgumentException failure =
                Assertions.assertThrows(IllegalArgumentException.class, config::validate);

        Assertions.assertTrue(failure.getMessage().contains("coordinator async concurrency"));
    }
