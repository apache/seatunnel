...
public class AbstractID implements Comparable<AbstractID>, java.io.Serializable {
    ...
    private static final long NUMBER_0XFFL = 0xFFL;
    ...
    public int hashCode() {
        final int move = 32;
        return ((int) this.lowerPart) ^ ((int) (this.lowerPart >>> move)) ^ ((int) this.upperPart) ^ ((int) (this.upperPart >>> move));
    }
    ...
    private static long byteArrayToLong(byte[] ba, int offset) {
        ...
        for (int i = 0; i < SIZE_OF_LONG; ++i) {
            l |= (ba[offset + SIZE_OF_LONG - 1 - i] & NUMBER_0XFFL) << (i << move);
        }
    ...
    private static void longToByteArray(long l, byte[] ba, int offset) {
        final int move = 3;
        for (int i = 0; i < SIZE_OF_LONG; ++i) {
            final int shift = i << move; // i * 8
            ba[offset + SIZE_OF_LONG - 1 - i] = (byte) ((l & (NUMBER_0XFFL << shift)) >>> shift);
        }
    }
}
