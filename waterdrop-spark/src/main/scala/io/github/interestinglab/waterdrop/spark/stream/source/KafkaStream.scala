...
class KafkaStream extends AbstractSparkStreamingSource[(String, String)] {
  ...
  override def getData(env: SparkStreamingEnv): DStream[(String, String)] = {

    inputDStream = KafkaUtils.createDirectStream(
      env.streamingContext,
