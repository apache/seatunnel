...
-  `-c` or `--config` 指定配置文件的路径

-  `-i` or `--variable` 指定配置文件中的变量，可以配置多个
...
例如: `-m yarn-cluster -ynm seatunnel` 指定作业上运行 `yarn`, 名称定义为  `yarn WebUI` 为 `seatunnel`
