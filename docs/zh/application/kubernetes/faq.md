...
`cancel` 删除 Job，并显式清理带 application 标签的 Pod、Service 和 ConfigMap。取消不创建 savepoint。资源删除后再次查询返回 `UNKNOWN`；Job TTL 到期后也会返回 `UNKNOWN`。
...
Master 退出码形成 Job 的 `Complete` 或 `Failed` condition。结束后的 Master Pod、ConfigMap 和 Service 保留到 `kubernetes.finished-job-retention-seconds` 指定的 TTL，便于排障。
...
- 限制 Job ConfigMap 的读取权限，不在配置和日志中暴露密钥。
