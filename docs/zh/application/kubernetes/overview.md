...
| 作业配置 | Secret | 以只读方式仅挂载给 Master，包含解析后的作业与部署信息 |
| 运行时配置 | 已有 ConfigMap | 以只读方式挂载给 Master 和 Worker，包含 `seatunnel.yaml` 与日志配置 |
...
- 已创建包含 SeaTunnel 运行时配置的 ConfigMap。
- Master 与 Worker Pod 之间允许 Hazelcast TCP 通信。
- 集群可以拉取包含 SeaTunnel、Kubernetes provider 和作业插件的镜像。
- 提交身份可以创建和管理 Job、Secret、Service 与 Pod。

## 推荐阅读顺序

| 阶段 | 文档 | 内容 |
| --- | --- | --- |
| 理解部署 | [集群架构](architecture.md) | Job、Worker Pod、Service、Secret、ConfigMap 和存储关系 |
