...
- active master 切换时，如果工作节点仍将某个固定槽位（`dynamic-slot: false`）报告为原作业所有，新 master 会直接复用该槽位，而不是重复申请。
- 只有工作节点地址、槽位 ID、工作节点实例序列号和所属作业 ID 都一致时才会复用。若槽位已经转给其他作业，恢复中的作业会将它视为不可用并走正常分配流程。
