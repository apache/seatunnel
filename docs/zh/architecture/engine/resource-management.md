...
```mermaid
flowchart TD
    jobmaster["JobMaster<br/>计算所需槽位<br/>表达资源需求（可选）<br/>应用标签过滤器（可选）"]
    registry["工作节点注册表<br/>WorkerProfile：总资源 / 可用资源 / 已分配槽位 / 未分配槽位"]
    strategy["分配策略<br/>Random / SlotRatio / SystemLoad"]
    slots["槽位管理<br/>分配槽位 / 释放槽位 / 跟踪使用情况"]
    workers["工作节点池<br/>Worker 1 / Worker 2 / Worker N<br/>每个节点维护多个 Slot"]

    jobmaster --> registry
    registry --> strategy --> slots --> workers
    ...
    class registry,strategy,slots layerCyan;
...
- active master 切换时，只要工作节点仍将某个槽位报告为原作业所有，新 master 就会直接复用该槽位，而不是重复申请；这项检查不区分 `dynamic-slot` 配置。固定槽位（`dynamic-slot: false`）是实际收益最明显的场景，因为重新申请一个工作节点仍持有的固定槽位本来会失败。
- 只有工作节点地址、槽位 ID、槽位分配序列号和所属作业 ID 都一致时才会复用。分配序列号在每次成功分配时都会更新，因此转给其他任务组的槽位会被恢复中的作业视为不可用，并走正常分配流程。
