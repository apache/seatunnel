...
- **引擎独立性**：将连接器逻辑与执行引擎解耦，使相同的连接器能够在 SeaTunnel Engine（Zeta）、Apache Flink 或 Apache Spark 上运行
- **高性能**：支持高吞吐、低延迟的大规模数据同步
- **容错性**：通过分布式快照和两阶段提交提供精确一次语义
- **易用性**：提供简单的配置方式和丰富的连接器生态系统（200+ 连接器）
...
| **连接器层** | 数据源/汇实现 | 200+ 连接器（JDBC、Kafka、CDC 等） |
