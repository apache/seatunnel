...
CoordinatorService 作业接纳线程池的核心线程数，默认值为 `10`。

**max-thread-num**

CoordinatorService 作业接纳线程池的最大线程数，默认值为 `2147483647`。
该线程池负责初始化新提交的作业并将其加入待调度队列。线程池直接交接任务，不使用工作队列；
所有线程繁忙且已达到配置的最大线程数时，提交可能被拒绝。

作业执行、完成回调、取消、检查点/保存点以及主节点切换恢复使用独立的生命周期线程池，
待调度作业的调度循环使用单独的线程。由于生命周期线程池仍同时执行阻塞等待及其所依赖的回调，
其最大线程数仍不设上限。因此，这些配置不限制运行中的作业数或主节点总线程数。
配置键和默认值保持不变，主节点重新激活后也使用相同配置。现有 `job_thread_pool_*` 指标描述作业接纳线程池，
`job_lifecycle_thread_pool_*` 指标单独描述生命周期与恢复线程池。
