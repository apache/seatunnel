...
使用 `schedule-strategy: WAIT` 时，等待资源的作业在主节点切换后保留彼此的排队顺序。
引擎在确认提交成功前，将入队序号写入作业的副本元数据。序号反映实际进入等待队列的顺序；
并发提交的作业初始化耗时不同，因此这个顺序可能与提交时间不同。

新主节点先恢复已经运行的作业，再按序号重建等待队列，随后才允许新提交的作业进入队列。
恢复中的等待作业轮到队首时才构建执行计划，查询状态不会触发整个等待队列的初始化。
取消某个等待作业不会改变其余作业的相对顺序。FIFO 约束的是资源准入顺序，不是作业完成顺序。

旧版本元数据没有入队序号。这类等待作业排在已有序号的等待作业之后，按初始化时间、作业 ID 排序，
并保存序号供后续故障恢复使用；其原来的排队顺序无法还原。该保证要求主节点支持入队序号，
不适用于仍有旧版本主节点参与的滚动升级阶段。恢复还依赖配置的状态副本或存储保留作业元数据。
