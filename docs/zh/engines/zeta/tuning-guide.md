...
Hazelcast 的 `SlowOperationDetector` 监控分区线程上的操作执行时间。当某个操作的执行时间超过配置的阈值（默认 10 秒，可通过 `hazelcast.slow.operation.detector.threshold.millis` 配置）时，会记录一条告警日志：
...
- 检查：启用 `org.apache.seatunnel.engine.server.checkpoint.CheckpointCoordinator` 的 DEBUG 日志（Checkpoint 完成日志在 DEBUG 级别），然后 grep `cost:` 查看 Checkpoint 耗时。
...
**配置位置：** `hazelcast.yaml` 中 `hazelcast.properties` 部分：
...
| `seatunnel.engine.checkpoint.interval` | `seatunnel.yaml` | **是**（节点重启） | 在节点启动时一次性解析，需重启生效 |
| `seatunnel.engine.checkpoint.timeout` | `seatunnel.yaml` | **是**（节点重启） | 在节点启动时一次性解析，需重启生效 |
| `seatunnel.engine.checkpoint.storage.*` | `seatunnel.yaml` | **是**（节点重启） | 在节点启动时一次性解析，需重启生效 |
| `seatunnel.engine.history-job-expire-minutes` | `seatunnel.yaml` | **是**（节点重启） | 在节点启动时一次性解析，需重启生效 |
