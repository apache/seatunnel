...
JSON 请求中，无法解析为 SeaTunnel 配置路径的键（例如正则字段名 `^t_nova_.*$`，或包含 `:`、`${...}` 等 HOCON 保留字符的键）会作为字面键处理。能够成功解析的键仍遵循原有路径解析语义：`a->b` 会生成嵌套配置，引号会被去除，未加引号的首尾空白会被裁剪；`job.mode` 这样的点号键仍是单个键。批量提交也遵循同一规则。
Json请求示例：
```json
{
    "env": {
