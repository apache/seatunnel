...
默认的 100 个基准任务保持不变。使用以下命令单独运行包含 12 个任务的公开改写测试集：
...
该测试集覆盖路由、CDC 前置条件以及连接器选项和模式选择，其中包含一个中文路由提示词。
它只运行改写任务，不会把原始任务一起计入分母。`--tiers` 按继承的复杂度层级筛选，
`--tasks` 使用独立 ID，例如 `t2_cdc_pg_kafka_p1`。无效、重复、空或不属于所选层级的
改写任务选择会在初始化模型提供方之前报错。生成、修复和验证流程仍沿用现有实现；
即使指定 `--level l1`，上述命令也会调用模型。

每个改写任务继承原始任务的全部断言和执行测试数据，并通过 `parent_sha256` 固定完整的
原始任务定义。原始任务发生变化时，必须重新检查语义等价性并显式更新指纹，不能静默
改变期望结果。保存的结果包含 `parent_id` 和 `parent_sha256`，`task_sha256` 则覆盖
展开后的完整改写任务，包括提示词和来源信息。

在不同输出目录中运行同一测试集的基准和候选版本，再使用 `benchmark.compare` 比较相同
改写任务 ID 的结果。不要把原始任务与改写任务直接配对，也不要将两者的通过率合并为独立证据。
新结果在运行级别记录 `suite`，不同测试集的比较会在任务配对前被拒绝。旧版未标记的
基准结果仍兼容，但未标记的改写结果需要重新运行。首个版本有意为每个原始任务保留
一个经过审查的改写提示词，更多改写属于后续测试集扩展。
现有默认提示词、评分、报告格式以及跳过验证层的排除规则均保持不变。

这是一个**公开回归测试集，而不是未见过的保留测试集**。离线测试只能验证任务选择、
测试数据继承、评分和报告契约，不能证明模型准确率、泛化能力或输出数据的完整语义等价性。
模型质量结论仍需要实际模型运行；生产提示词调整应与测试集变更分开进行。
