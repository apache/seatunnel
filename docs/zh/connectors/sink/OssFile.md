...
2. 您必须确保`${SEATUNNEL_HOME}/plugins/`目录中有`hadoop-aliyun-xx.jar`、`aliyun-sdk-oss-xx.jar`和对应的 JDOM jar。`hadoop-aliyun`的版本需要与您在spark/flink中使用的hadoop版本一致，`aliyun-sdk-oss`和 JDOM 需要使用该`hadoop-aliyun`版本所依赖的版本。例如:`hadoop-aliyun-3.4.3.jar`依赖`aliyun-sdk-oss-3.13.2.jar`和`jdom2-2.0.6.1.jar`，而`hadoop-aliyun-3.1.4.jar`依赖`aliyun-sdk-oss-3.4.1.jar`和`jdom-1.1.jar`。

### 适用于SeaTunnel Zeta引擎

1. 您必须确保在`${SEATUNNEL_HOME}/lib/`目录中有`seatunnel-hadoop3-3.4.3-uber.jar`、`aliyun-sdk-oss-3.13.2.jar`、`hadoop-aliyun-3.4.3.jar`和`jdom2-2.0.6.1.jar`。

> 这四个 jar 必须一起升级。`hadoop-aliyun` 3.4.3 引用了`com.aliyun.oss.model.ListObjectsV2Request`/`ListObjectsV2Result`，并要求四个`OSSClient`方法返回`VoidResult`；`aliyun-sdk-oss` 3.4.1 两者都不具备，而`aliyun-sdk-oss` 3.13.2 使用`jdom2`而非`jdom` 1.x 解析 XML。将旧的`aliyun-sdk-oss`/`jdom` jar 与 3.4.3 uber jar 混用在构建时不会报错，但运行时每次 OSS 列举操作都会失败。
...
| 名称                                    | 类型      | 必需 | 默认值                                        | 描述                                                                |
|---------------------------------------|---------|----|--------------------------------------------|-------------------------------------------------------------------|
| path                                  | string  | 是  | -                                          | Sink 写入的 OSS 路径。配合 `bucket`，实际位置为 `oss://<bucket><path>`。                                                                                                       |
...
| merge_update_event                    | boolean | 否  | false                                      | 仅当file_format_type为canal_json、debezium_json、maxwell_json 时使用。                                                                                |
