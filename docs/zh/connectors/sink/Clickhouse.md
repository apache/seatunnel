...
当上游是变更日志类数据源（例如 MySQL CDC、PostgreSQL CDC）时，行中会包含 `INSERT`、`UPDATE_BEFORE`/`UPDATE_AFTER`
和 `DELETE` 事件。需要配置 `primary_key`，让接收端能够把 `UPDATE`、`DELETE` 行映射到正确的目标行；当目标表自身
没有去重引擎时，配合 `support_upsert = true` 可以改为查询主键后再写入。
