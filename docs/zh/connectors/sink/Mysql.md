...
| xa_data_source_class_name    | String  | 否       | -                            | 数据库 Driver 的 xa 数据源类名，例如 MySQL 是 `com.mysql.cj.jdbc.MysqlXADataSource`，<br/>其他数据源请参考对应驱动文档。                                                                     |
...
| enable_upsert                | Boolean | 否       | true                         | 通过 `primary_keys` 存在启用 upsert，如果任务只有 “插入”，将此参数设置为 “false” 可以加快数据导入                                                                                                                 |
