...
- **PostgreSQL**：`fillfactor` 会生成 `WITH (fillfactor=...)`；`tablespace` 会生成 `TABLESPACE "..."`。仅接受上述 curated key（不支持任意 `WITH` 参数）。OpenGauss、HighGo 通过 Postgres catalog/dialect 继承同一套校验与 DDL。不支持的取值会在执行 `CREATE TABLE` 时报错，而非在作业提交阶段校验。
