...
- 当轮转后的文件仍位于配置路径下并且符合文件过滤条件时，支持重命名后新建文件的轮转方式。检测到 copy-truncate 重写时，从配置的 header 边界重新读取。
...
- 文件 key 是文件系统内的标识，并非永久标识。删除文件后重用 inode 或重新挂载文件系统可能使该标识假设失效。恢复任务时应保持同一 source 文件系统；采样内容锚点可以检测部分文件替换，但不能保证识别所有重写或 inode 重用。
- 内容锚点最多采样已提交偏移量之前的前 2 KiB 和后 2 KiB。它可以检测常见的 copy-truncate 重写，但不会检查两个采样区间之间的内容。reader 在读取前后也会检查已分配范围的结束锚点。读取前发现过期范围时，不推进偏移量；读取开始后发现变化时，任务失败且不确认该范围，已发送的行无法撤回。并发重写不具备原子读取保证，因此可靠的 tailing 仍要求输入文件仅追加内容。
- enumerator 和 reader 节点必须能看到相同的文件和文件标识。如果它们可能运行在不同节点，请使用共享挂载目录。
- 文件消失后，其状态会保留三个成功的扫描周期；如果该文件没有等待或运行中的 split，之后会删除该状态。
- 如果已分配的文件在读取前的文件标识或内容检查期间消失，则丢弃该范围且不推进偏移量。其他检查或读取 I/O 错误（包括权限不足）会使任务失败，而不是静默跳过数据。重启前应修复文件系统或权限问题。

`latest` 模式会在检查文件内容前，按稳定文件标识记录首次列举的结束偏移量；重命名后保留该偏移量，被替换的新文件不会继承它。初始文件暂时不可读时，后续发现的新文件不会被跳过。如果无法检查初始文件标识，则不假设初始偏移量；文件恢复可读后，从配置的 header 边界读取，这可能包含已有行。如果初始文件尚未包含配置的全部 header 行，初始偏移量会通过 checkpoint 保留，直到 header 边界完整，再正常读取后续数据行。
