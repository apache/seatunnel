...
Oracle Source 本质上是一个批连接器，但可以通过 `partition_column` 配合 `partition_lower_bound` 和 `partition_upper_bound` 在流式作业中做增量区间读取：每个 checkpoint 都会重放配置的区间。这种用法只适合范围小且有界，或按计划重放历史数据的场景。持续变更捕获请改用 Oracle-CDC。
