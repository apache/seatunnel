...
| schema-changes.enabled                    | Boolean  | 否   | false    | 启用 schema evolution 事件。设置为 `true` 后，PostgreSQL CDC 可以将 `ADD COLUMN`、`DROP COLUMN`、`RENAME COLUMN` 和 `ALTER COLUMN ... TYPE ...` 事件传递给下游。                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
> 当前 schema evolution 仅支持列的新增、删除、重命名和类型变更。
