...
| driver                                     | String        | 是   | -               | 用于连接 PostgreSQL 的 JDBC 类名，值为 `org.postgresql.Driver`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| username                                   | String        | 否   | -               | 连接实例的用户名                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| password                                   | String        | 否   | -               | 连接实例的密码                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| query                                      | String        | 是   | -               | 查询语句                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| connection_check_timeout_sec               | Int         | 否   | 30              | 用于验证连接的数据库操作完成的等待时间（秒）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| partition_column                           | String        | 否   | -               | 用于并行读取拆分的列名，只能配置一列。PostgreSQL 通过共享 JDBC 拆分器支持数字、字符串和日期/时间类型拆分列。                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| split.string_split_mode                    | Enum       | 否   | sample          | 字符串拆分算法。`sample` 表示通过采样字符串值进行拆分；`charset_based` 假设拆分列使用可打印 ASCII 字符。 |
| split.string-strategy                      | Enum       | 否   | -               | 字符串分区列的拆分策略。`range` 表示范围拆分，`hash` 表示哈希拆分，`none` 表示禁用字符串拆分，`auto` 表示优先尝试范围拆分，必要时回退到哈希拆分。 |
| split.string_split_mode_collate            | String     | 否   | -               | 当 `split.string_split_mode = charset_based` 且表使用特殊排序规则时的 collation 配置；未配置时使用数据库默认 collation。 |
## 并行读取器
