...
| url                                            | String   | 是       | -       | URL 必须包含数据库，如 "jdbc:sqlserver://localhost:1433;databaseName=test"。URL 中的 SQL Server JDBC 连接属性（如 `encrypt`、`trustServerCertificate`）会传递给 CDC 连接；`debezium` 中显式配置的属性（如 `database.trustServerCertificate`）优先。 |
...
| schema-changes.behavior                        | Enum     | 否       | evolve  | `schema-changes.enabled = true` 时，处理经过 `schema-changes.include` / `schema-changes.exclude` 过滤后保留的事件。值不区分大小写：`strict`、`evolve`、`ignore`。`ignore` 只丢弃纯注释事件，行结构变更会失败。支持 Zeta 和 Flink，不支持 Spark。详见 [Schema 变更行为策略](../../introduction/configuration/schema-evolution.md#schema-变更行为策略)。 |
