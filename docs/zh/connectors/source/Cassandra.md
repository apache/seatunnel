...
`--dry-run connect` 连接 Cassandra，并预编译每条配置的 SELECT 以获取投影后的 schema，不执行数据查询。单表和 `tables_configs` 使用与正常执行相同的列类型转换和表标识，不创建数据读取器。

CQL 必须以 SELECT 开头（不区分大小写，允许前导空白）；不支持前导注释、非 SELECT 语句或未绑定参数。驱动可能读取系统元数据并更新预编译语句缓存，但连接器不读取业务行或执行写入。预编译成功不代表具备 SELECT 权限，也不保证一致性级别或后续执行可用。

连接、元数据和请求的单次等待上限为 10 秒，初始化不会无限重连。这不是跨所有节点和表的全局超时。其他驱动配置保持不变，正常作业执行不变。
