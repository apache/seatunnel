...
FakeSource 是一个虚拟数据源，它根据用户定义的 schema 数据结构随机生成指定数量的行数据，主要用于类型转换、连接器新功能测试和本地管道验证等场景。
...
> 此示例随机生成指定类型的数据。如果您想了解如何声明字段类型，请点击 [这里](../../introduction/concepts/schema-feature.md#如何声明支持的类型)。
