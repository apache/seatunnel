...
| queue_full_retry_after_ms | Integer | 否 | 500 | `QUEUE_FULL` 响应中建议的退避毫秒数（格式 `QUEUE_FULL:<ms>`）。必须大于 0。 |
...
`EdgeSocket` 绑定的是它实际运行的 Worker 节点端口。如果 Job 重启后 Source Task 漂移到另一台 Worker，采集器配置的目标地址就会失效。

应对方式取决于部署环境：

- K8s — 使用 `LoadBalancer` Service（推荐）。详见下方 [K8s 部署](#k8s-部署)。
- VM / 裸机 — 使用 Zeta 的 `tag_filter` 将 Source 固定到指定 Worker。示例见下方折叠区域。
...
调度器只会在 `member-attributes` 与 `tag_filter` 完全匹配的 Worker 上分配 Slot。
...
| Worker 绑定了 VPC EIP 或 NAT | 是（通过公网地址） | - | 采集器连接 EIP/NAT 地址；设置 `endpoint = "<eip>:<port>"` 方便日志可见。 |
...
- `ACK` — 认证通过，可以开始发送数据。
- `AUTH_FAILED` — Token 错误，采集器应重新连接并携带正确 Token。
...
`RECEIVED` 仅表示批次已入队，不表示作业 checkpoint 已完成；在下一次已完成的作业 checkpoint 之前若 Worker 重启，内存队列中的数据可能丢失。`__COMMIT__` 为可选，采集端可以只使用 `__BATCH__` → `RECEIVED`。若选用 `__COMMIT__`，可在收到 `ACK:<watermark>` 前保留本地缓冲并以 Source 返回的水位清理缓冲。
...
下表列出连接阶段可能遇到的结果，以及 Source 在应用层可能返回的全部响应；「采集端操作」列给出对应处理建议。

| 响应 | 触发请求 | 含义 | 采集端操作 |
|---|---|---|---|
| `ACK` | `__AUTH__` | 认证通过。 | 开始发送批次数据。 |
| `AUTH_FAILED` | `__AUTH__` | Token 错误或缺失。 | 断开后携带正确 Token 重连。 |
| `Connection refused` | 连接阶段 | 服务端 Socket 已挂起，连接在 TCP 层被拒绝（见[连接](#连接)）。 | 原因多样，先排查再决定是否重连。 |
| `REJECTED` | 连接阶段 | 在 TCP backlog 竞争窗口内被接受，已有其他采集器连接中（见[连接](#连接)）。 | 立即停止，确认是否有其他采集器实例在运行。不要自动重试。 |
| `RECEIVED` | `__BATCH__` | 记录已接收并成功入队。 | 继续发送；若需与 checkpoint 水位对齐本地缓冲，`__COMMIT__` 可选，可轮询至 `ACK` 并按返回水位丢弃缓冲。 |
| `QUEUE_FULL:<ms>` | `__BATCH__` | 队列达到背压高水位（见[背压](#背压)）。 | 至少等待 `<ms>` ms 后原样重发同一批次。 |
| `RETRY` | `__BATCH__` 或 `__COMMIT__` | 物理队列已满（少见）或请求格式非法。 | 格式错误：修正后重发；物理满队：指数退避后原样重发同一批次。 |
...
>
> `ACK:<watermarkBatchId>` 说明：返回值是 Source 当前的 checkpoint 水位，可能高于查询时传入的 `batchId`。采集端应以返回的水位数字为准做缓冲清理。
...
阶段 ③ 与 ④ 可交错；④ 仅在使用 `__COMMIT__` 时需要，**可不实现**，仅 `__BATCH__` → `RECEIVED` 亦合法。**不必**等 `ACK` 再发下一批 `__BATCH__`。
...
队列中存储的是经过协议处理后的 `byte[]`。当 `packet_mode = PACKET` 且启用压缩（`compression = GZIP / ZLIB / DEFLATE`）时，入队的是压缩后的字节，实际大小可能远小于原始 payload。

评估时应以入队大小（即压缩后大小）为准。如果不确定，可从采集端抽取若干样本消息，计算压缩后的平均大小作为基准。

:::

建议将 checkpoint 状态控制在 10 MB 以内。checkpoint 状态会通过 Hazelcast IMap 在集群中复制，过大的状态会增加内存占用和网络开销，具体阈值取决于集群的硬件配置和网络带宽。
