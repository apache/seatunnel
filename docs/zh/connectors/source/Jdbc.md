...
| split.sample-sharding.threshold            | Int     | 否    | 1000            | 为兼容性保留，并在近似行数不可用时用于稀疏区间回退的边界控制。不均匀动态分片在预估分片数超过该阈值时**不再自动**触发全列客户端采样，改为 Index Probing。默认值为 1000。                                                                                                                 |
| split.inverse-sampling.rate                | Int     | 否    | 1000            | 为兼容旧采样相关配置而保留。大表动态分片默认走 Index Probing，不再默认全列客户端采样。默认值为 1000。                                                                                                            |
| split.allow-sampling                       | Boolean | 否    | true            | 为兼容性保留。大表不均匀动态分片默认使用 Index Probing，不再自动执行全列客户端采样。                                                                                                                                                                                              |
...
动态分片采用惰性生成：均匀键用算术游标，非均匀键用 Index Probing；Enumerator 最多保留 `split.max-pending-splits` 个已生成未分配的 split，并按 `split.assign.batch-size` 分批下发。若 Reader 在两次下发之间空闲，可调大这两个参数；它们**不是**作业总 split 数硬上限。
