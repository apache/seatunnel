...
| split.string-strategy                      | String  | 否    | -               | 控制 String 分区列的分片方式，可选值为 `none`、`hash`、`range`、`auto`。`range` 仅在方言验证排序语义后启用：MySQL 要求列使用 binary collation 且键值为固定长度的可打印 ASCII 字符串；Oracle 要求 `NLS_SORT=BINARY`、`NLS_COMP=BINARY`、列的 collation 为 `BINARY` 或 `USING_NLS_COMP`、数据库字符编码保持可打印 ASCII 的二进制排序，以及相同的键值限制。Oracle 会在每个 reader 连接上重新验证会话条件；若 reader 会话不安全，会在执行 range 条件前失败关闭。`range` 会拒绝不支持或不安全的配置。对于支持 range 分片的方言，`auto` 在验证通过时使用 range，否则回退为 hash；若方言不支持 hash，则使用单分片。没有经过验证的 range 支持的方言会拒绝 `range` 和 `auto`。未设置该参数时，SeaTunnel 保持现有 `split.string_split_mode` 行为。 |
