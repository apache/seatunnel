...
当只配置 `query`，且 JDBC 报告所有结果列均来自同一张物理表（例如 `SELECT * FROM db.table`）时，SeaTunnel 会根据查询结果元数据解析该表，并把底表的元数据（字段注释、表注释、主键、约束键、分区键）合并进查询推导出的 schema，效果等同于同时配置 `table_path` 与 `query`。查询中被改名或经过表达式加工的列保留查询推导的定义。检测到多表查询，或驱动未提供足够的列来源元数据以确认单表时，会跳过合并。已有的查询主键推断还存在独立限制，详见 [Query 与主键注意事项](#query-与主键注意事项)。
