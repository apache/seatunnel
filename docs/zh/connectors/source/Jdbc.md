...
| 自定义读取列、JOIN 或数据库表达式 | `query`，可以同时配置 `table_path` | SeaTunnel 执行用户提供的 SQL。当 JDBC 元数据能识别出单个物理表时，会自动保留匹配的列注释和表注释。对于 JOIN、视图、表达式或任何需要明确表身份和元数据的查询，请同时配置 `table_path`。部分 JOIN 无法安全推断查询主键，详见 [Query 与主键注意事项](#query-与主键注意事项)。 |
...
| query                                      | String  | 否    | -       | 要执行的 SQL。对于能识别为单个物理表的查询，会自动保留匹配的表注释和列注释。需要同时明确表身份和元数据时，可以与 `table_path` 组合使用。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
