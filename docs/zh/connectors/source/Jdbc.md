...
| split.assign.batch-size                    | Int     | 否    | 16      | 每次分配给单个 Reader 的最大 split 数量。Reader 本地队列不足时会继续请求下一批。该参数用于限制 Reader 侧 pending split 元数据与 checkpoint 体积，不改变 split 谓词与切分结果。                                                                                                                                                                                                                                                                                                                                                                      |
...
`split.assign.batch-size` 控制每次向单个 Reader 下发多少个已生成的 split；剩余 split 保留在 Enumerator 中，待 Reader 继续请求后再下发。该参数不改变分片边界与数据覆盖范围。
