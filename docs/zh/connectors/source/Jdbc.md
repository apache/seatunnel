...
| split.assign.batch-size                    | Int     | 否    | 16      | 每次向单个 Reader 下发的 split 数量上限。Reader 本地队列不足时会通过 `handleSplitRequest` 继续拉取。该参数约束 Reader 侧挂起的 split 元数据与 checkpoint 体积，不改变分片谓词。 |
| split.max-pending-splits                   | Int     | 否    | 64      | Enumerator 侧「已生成但尚未分配」的 split 数量上限。低于水位时会惰性继续生成。用于限制协调器内存，**不是**作业总 split 数硬上限。 |
| split.even-distribution.factor.lower-bound | Double  | 否    | 0.05            | 不建议修改。<br/> 分片键分布因子的下界。该因子用于判断表数据是否均匀分布。如果计算出的分布因子大于或等于此下界（即 (MAX(id) - MIN(id) + 1) / 行数），则将优化为均匀分布的分片方式。否则表将被视为非均匀分布，并使用 **Index Probing**（迭代 `queryNextChunkMax`）划界。默认值为 0.05。  |
| split.even-distribution.factor.upper-bound | Double  | 否    | 100             | 不建议修改。<br/> 分片键分布因子的上界。该因子用于判断表数据是否均匀分布。如果计算出的分布因子小于或等于此上界（即 (MAX(id) - MIN(id) + 1) / 行数），则将优化为均匀分布的分片方式。否则表将被视为非均匀分布，并使用 **Index Probing** 划界。默认值为 100.0。 |
| split.sample-sharding.threshold            | Int     | 否    | 1000            | 为兼容性保留，并在近似行数不可用时用于稀疏区间回退的边界控制。不均匀动态分片在**生产规划路径**上不再执行全列客户端采样，改为 Index Probing。默认值为 1000。                                                                                                                 |
| split.inverse-sampling.rate                | Int     | 否    | 1000            | 为兼容旧采样相关配置而保留。全列客户端采样不在生产动态分片规划路径中使用。默认值为 1000。                                                                                                            |
| split.allow-sampling                       | Boolean | 否    | true            | 为兼容性保留。生产动态分片规划路径已无法走到全列客户端采样；不均匀键无论该开关如何均使用 Index Probing。                                                                                                                                                                                              |
...
动态分片对数值键与非 charset 字符串键采用惰性生成：均匀键用算术游标，非均匀键用 Index Probing；Enumerator 最多保留 `split.max-pending-splits` 个已生成未分配的 split，并按 `split.assign.batch-size` 分批下发。若 Reader 在两次下发之间空闲，可调大这两个参数；它们**不是**作业总 split 数硬上限。DATE 分片键与 charset-based STRING 分片键在本版本仍会预先物化完整 split 列表。
