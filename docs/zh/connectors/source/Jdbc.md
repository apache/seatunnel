...
对于**复合主键**表（`PRIMARY KEY (a, b, ...)`），动态分片会以**元组形式使用全部主键列**分片（如 `(a, b) > (?, ?)`，以可移植的展开 `OR`/`AND` 形式生成 SQL）。这样即使首个键列大量重复，也能依靠其余键列切分成均衡的分片。该行为在启用动态分片（默认开启）、方言支持复合分片且所有键列均为受支持的分片键类型时生效；否则与之前一致，回退为使用第一个受支持的主键列分片。目前复合分片仅对 **MySQL、PostgreSQL、SQLite、SQL Server 和 Oracle** 启用。边界查询使用了 `FETCH FIRST`/`OFFSET` 分页语法，连接器会在运行时检测数据库版本：在 12c 之前的 Oracle 版本上，连接器会自动回退为单列分片；其他方言保持单列分片行为。

复合主键中包含 `NULL` 分量的行（SQLite 允许复合主键列中出现 `NULL`）不会参与分片边界的计算，而是通过显式谓词归入**第一个分片**，因此每一行（包括含 `NULL` 键分量的行）都会被恰好读取一次。

**关闭复合分片（保持旧行为）**：如果希望对复合主键表保持之前的单列分片行为，可以将 `partition_column` 显式设置为某一个主键列（例如第一个键列）。显式配置的 `partition_column` 始终优先于复合主键分支，表会像引入复合分片之前一样按该单列切分。也可以设置 `enable_concurrent_read = false` 完全跳过分片分析，整表只使用一个分片读取。

**已知限制 — 字符串排序规则（collation）**：复合分片路径假设数据库端的 `ORDER BY` 与 Java 端的元组比较结果一致。STRING 列使用 Java `String.compareTo`（Unicode 码点）语义比较，这可能与大小写或重音不敏感的数据库排序规则（如 MySQL 的 `utf8mb4_general_ci`）不一致。因此，STRING 列使用与二进制/码点顺序不一致的排序规则的复合主键，其 STRING 分量不受支持：Java 端计算出的分片边界可能与数据库返回的行序不匹配，导致相关分片之间出现行重复或行丢失。对于此类表，建议优先使用数值型的首键列，或通过 `partition_column` 回退为单列分片。
