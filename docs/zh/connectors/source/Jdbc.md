...
| split.string-strategy                      | String  | 否    | -               | 控制 String 分区列的分片方式，可选值为 `none`、`hash`、`range`、`auto`。`range` 仅在方言验证排序语义后启用：MySQL 要求列使用 binary collation 且键值为固定长度的可打印 ASCII 字符串；Oracle 要求 `NLS_SORT=BINARY`、`NLS_COMP=BINARY`，并满足相同的键值限制。`range` 会拒绝不支持或不安全的配置。`auto` 在验证通过时使用 range，否则回退为 hash；若方言不支持 hash，则使用单分片。未设置该参数时，SeaTunnel 保持现有 `split.string_split_mode` 行为。                                                                                                                                                                                                                                                                       |
