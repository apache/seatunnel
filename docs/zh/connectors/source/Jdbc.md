...
当只配置 `query`，且 JDBC 报告所有结果列均来自同一张物理表（例如 `SELECT * FROM db.table`）时，SeaTunnel 会根据查询结果元数据解析该表，并默认把底表的字段注释、表注释和表选项合并进查询推导出的 schema，使带自动建表能力的 Sink 能保留它们。默认合并有意排除主键、约束键和分区键，因为携带它们会改变运行时行为：配置了 `generate_sink_sql` 的 Sink 会从普通 insert 切换为 upsert，切分规划也可能根据合并出的主键推断切分列。配置 `query_table_metadata_merge = ALL` 可以同时合并这些键——效果等同于同时配置 `table_path` 与 `query`；配置 `NONE` 则完全跳过底表解析。查询中被改名的列保留查询推导的定义。注意：计算列或表达式列（例如 `UPPER(name)`）通常不携带来源元数据，这会导致整个查询无法确认单表——此时会完全跳过合并，而不仅是跳过该列。检测到多表查询时同样跳过合并；单表验证或底表加载失败时回退为查询推导的 schema，并输出 warn 日志。已有的查询主键推断还存在独立限制，详见 [Query 与主键注意事项](#query-与主键注意事项)。
...
| query_table_metadata_merge                 | Enum    | 否    | COMMENT | 仅配置 `query` 的源表合并底表元数据的范围（仅在查询被验证为映射到单张物理表时生效）。`COMMENT` 合并字段注释、表注释和表选项；`ALL` 额外合并主键、约束键和分区键——效果等同于同时配置 `table_path` 与 `query`，可能使配置了 `generate_sink_sql` 的 Sink 从 insert 切换为 upsert，并启用基于主键的切分规划；`NONE` 完全跳过底表解析。也可以在 `table_list` 中按表单独配置。                                                                                                                                                                                                                                       |
