...
对于**复合主键**表（`PRIMARY KEY (a, b, ...)`），动态分片会以**元组形式使用全部主键列**分片（如 `(a, b) > (?, ?)`，以可移植的展开 `OR`/`AND` 形式生成 SQL）。这样即使首个键列大量重复，也能依靠其余键列切分成均衡的分片。该行为在启用动态分片（默认开启）、方言支持复合分片且所有键列均为受支持的分片键类型时生效；否则与之前一致，回退为使用第一个受支持的主键列分片。目前复合分片仅对 **MySQL、PostgreSQL、SQLite、SQL Server 和 Oracle** 启用（Oracle 需要 12c Release 1 及以上版本，因为边界查询使用了 `FETCH FIRST`/`OFFSET` 分页语法）；其他方言保持单列分片行为。
