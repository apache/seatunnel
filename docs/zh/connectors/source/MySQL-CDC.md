...
| schema-changes.behavior                   | Enum     | 否    | evolve  | 当 `schema-changes.enabled = true` 时，控制已发出的 schema change event 的处理方式。有效值为 `strict`、`evolve` 和 `ignore`。详见 [Schema 变更行为策略](../../introduction/configuration/schema-evolution.md#schema-变更行为策略)。                                                                                                      |
