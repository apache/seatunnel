...
| startup.specific-offset.file              | String   | 否    | -       | 从指定的binlog日志文件名开始. 当 `startup.mode` 为 `specific` 时，需要和 `startup.specific-offset.pos` 一起使用，且不能和 `startup.specific-offset.gtid-set` 一起使用.                                                                                                                                     |
| startup.specific-offset.pos               | Long     | 否    | -       | 从指定的binlog日志文件位置开始. 当 `startup.mode` 为 `specific` 时，需要和 `startup.specific-offset.file` 一起使用，且不能和 `startup.specific-offset.gtid-set` 一起使用.                                                                                                                                   |
| startup.specific-offset.gtid-set          | String   | 否    | -       | 当 `startup.mode` 为 `specific` 时，从指定的 MySQL GTID 集合开始. 该选项不能和 `startup.specific-offset.file` 或 `startup.specific-offset.pos` 一起使用.                                                                                                                                             |
