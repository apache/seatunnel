...
整型（`TINYINT`、`SMALLINT`、`INT`、`BIGINT`）与 `DECIMAL` 之间的比较保留数值精度。
此规则适用于 `=`、`!=`、`<>`、`<`、`<=`、`>`、`>=` 以及数值的 `IN` 和 `NOT IN` 判断。
例如，`BIGINT` 值 `9007199254740993` 不会匹配 `WHERE id = 9007199254740992`。

当任一操作数为 `FLOAT` 或 `DOUBLE` 时，仍使用浮点转换进行比较。
如需精确比较，请使用整型或 `DECIMAL` 操作数，而非浮点操作数。
此行为适用于 `ZETA` 和 `INTERNAL` 两种 SQL 引擎设置。
