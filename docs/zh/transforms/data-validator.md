...
> DataValidator：按规则校验字段值，并处理不符合要求的数据

## 描述

DataValidator 转换插件会根据配置规则校验字段值，并按照指定的错误处理策略处理验证失败的数据。它支持空值检查、范围验证、长度验证和正则表达式匹配等多种校验方式。
...
**注意**：`ROUTE_TO_TABLE` 模式仅适用于支持多表写入的 sink 连接器。sink 必须具备把数据路由到不同目标表的能力。
...
数组长度为元素个数，包含空元素。嵌套数组只计算最外层的长度，二进制值按字节数计算。
字符串长度使用 Java UTF-16 代码单元数，集合长度使用集合大小。
空值通过此规则；如需拒绝空值，请组合使用 `NOT_NULL`。
