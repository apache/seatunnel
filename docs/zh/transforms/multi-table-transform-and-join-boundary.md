...
| 能力 | 是否支持 | 说明 |
|---|---|---|
| 按表字段重命名 / 映射 | ✅ 支持 | 使用带 `source_table_name` 的 `FieldMapper` |
| 按表列过滤 | ✅ 支持 | 使用带 `source_table_name` 的 `Filter` |
| 按表类型转换 | ✅ 支持 | 使用 `FieldMapper` 的 `define_sink_type` 选项 |
| 单表 SQL Transform | ✅ 支持 | 使用带特定 `source_table_name` 的 `SQL` Transform |
...
| 按表 JSON 嵌套字段提取 | ✅ 支持 | 使用带 `source_table_name` 的 `JsonPath` Transform |
...
`TableMerge` **将多张表的行流合并为一张结果表**。所有输入表必须具有相同（或兼容）的 Schema。
通常用于将多表路由至同一个 Sink。
...
在单个 SQL Transform 中引用多个 `source_table_name` 的配置不被支持，会导致配置报错。
...
当你需要对同一 Source 的不同表应用不同的 Transform 时，可声明多个具有不同
`source_table_name` 的 Transform 块：
...
此模式仅在三张表（`orders`、`payments`、`refunds`）都包含 `created_at` 和 `event_type`
字段时有效。若各表 Schema 不同，`TableMerge` 会在运行时报错。

---

## 7. 多表 Transform 的 EtLT 模式

**EtLT**（Extract、轻量 transform、Load，再在数仓中 Transform）是当 SeaTunnel Transform
层无法完成全量转换需求时的推荐架构模式：
...
不能直接实现。可以先用 `TableMerge` 将 Schema 兼容的表合并，再对合并结果应用单一 Transform。
若各表 Schema 不同，则必须为每个 `source_table_name` 显式配置。
...
未匹配的表会继续在流水线中流动，可被通过 `source_table_name` 或 `table_match_regex`
匹配的 Sink 捕获。
