...
Reader 进度在成功处理记录后采样独立的 offset 坐标，通常每秒最多一次；首次成功发送、split
或 Reader 生命周期变化时立即采样。采样间隔使用单调时钟，报告时间戳仍为 epoch 时间戳。
轮询不会读取 Connector 的可变 offset 对象，因此后续原地更新或发送失败不会推进已发布的位置。
批次部分失败时保留最后一个已验证的样本，不保证包含失败前的每条成功记录。精确坐标仅证明该次
成功采样的位置，不保证实时性或 checkpoint 完成。若之后没有记录成功，样本可以一直保持旧值。
`lastPositionChangeAt` 表示观察到采样位置变化的时间。仅选中的样本复制坐标，记录路径仍有少量
采样条件检查开销。

Enumerator 计数不一致时报告 `BEST_EFFORT`，不改变分配或 checkpoint 行为。普通诊断转换异常
发布不可用的计数，避免将之前的精确分配快照当作新状态，不使分配或恢复失败。尚未完成分块的表计数
包括当前正在分块的表，但不会为此将该表保留在 assigner 的生产队列中。
