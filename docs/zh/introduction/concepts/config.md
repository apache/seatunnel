...
可以同时定义多个source。目前支持的source请看[SeaTunnel Source 连接器](../../connectors/source)。每种source都有自己特定的参数用来
...
与source类似，transform也有属于每个模块的特定参数。目前支持的 transform 请看 [SeaTunnel 数据转换](../../transforms)。
...
sink模块，你可以快速高效地完成这个操作。Sink和source非常相似，区别在于读取和写入。所以去看看我们[SeaTunnel Sink 连接器](../../connectors/sink)吧。
