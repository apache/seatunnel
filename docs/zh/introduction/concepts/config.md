...
变量使用方法：
 ...
 - `${varName:default}`，如果变量未传值，则使用默认值。配置文件中多个相同的key默认值也必须相同。默认值如果包含占位符，不会被解析。顶层的source/transform/sink节点和插件所在的节点不允许直接设置json默认值。如果设置默认值，普通变量需要写在双引号中。如果默认值是map类型，json格式变量值需要写在三引号中，如下：
