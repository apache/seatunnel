...
- **行为修正**：Zeta SQL 引擎比较整数和 DECIMAL 数组元素时不再转换为 `double`。
  例如，对 BIGINT 值 `[9007199254740992, 9007199254740993]` 使用 `ARRAY_MAX`，
  现在返回 `9007199254740993`，而不是第一个较小的值。DECIMAL 极值比较同样保留
  之前丢失的精度差异。
- **影响**：依赖舍入比较结果的作业可能选择不同的值。声明的输出类型、null 处理和
  浮点数组排序规则保持不变。返回所选 DECIMAL 元素时不会调整其小数位数；选择不同的
  元素也可能选择不同的原始小数位数。不需要迁移配置或状态；请检查依赖旧错误结果的
  下游预期值。
