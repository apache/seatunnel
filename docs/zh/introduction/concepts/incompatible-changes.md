...
- **行为变更：运行中作业的指标改为有界的尽力收集**
  ...
  - **影响**：成功响应也可能只包含部分指标。监控系统应将缺失的工作节点指标视为暂时不可用，而不是数值为零。主节点等待时间默认是 3 秒，可通过 `seatunnel.engine.metrics-fetch-timeout-ms` 调整；该限制不会取消已经提交的 Hazelcast 工作节点调用，调用仍由 Hazelcast 的操作调用超时负责回收。
