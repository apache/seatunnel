...
固定桶 Sink 现在将同一物理分区和 bucket 路由到同一个 Writer。请设置 `multi_table_sink_replica = 1`，拆开固定桶与其他桶模式的 Sink 定义；多个源流指向同一物理表时，应先合并再写入一个 Sink。此前接受的冲突配置现在会在初始化时失败。分区器使用启动时的源表和目标表 schema；本次修改不新增完整在线 DDL 处理。Schema 控制行保留目标 subtask，不按数据行转换。Checkpoint 序列化格式不变，本修复不提供额外的全局恢复或扩缩容保证。
