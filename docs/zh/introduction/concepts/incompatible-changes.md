...
整型与 `DECIMAL` 值之间的 SQL 比较及 `IN`/`NOT IN` 判断不再先将操作数转换为 `double`。
因此，涉及大整数或高精度小数的过滤条件可能选择与此前不同的行。
请检查相关条件；如果已写入的数据依赖此前的舍入比较结果，应核对并修正这些数据。
无需更改配置、输出结构或状态格式。涉及 `FLOAT` 或 `DOUBLE` 的比较保持原有行为。
