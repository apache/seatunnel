...
| json         | `java.lang.String`                                 | 以 JSON 文本保存的合法 JSON 值。JSON 格式会将其序列化为 JSON 值，而不是转义后的字符串。Java `null` 表示 SQL 空值，字符串 `null` 表示 JSON null 值。连接器必须提供明确的原生类型映射后才能支持该类型。                                                                                                                              |
...
| array        | `ValueType[]`                                      | 数组是一种表示元素集合的数据类型。 元素类型包括： `int` `string` `json` `boolean` `tinyint` `smallint` `bigint` `float` `double`.                                                                                                                                                                                                       |

#### 如何声明支持的类型

SeaTunnel 提供了一种简单直接的方式来声明基本类型。基本类型的关键字包括：`string`, `json`, `boolean`, `tinyint`, `smallint`, `int`, `bigint`, `float`, `double`, `date`, `time`, `timestamp`, 和 `null`。基本类型的关键字名称可以直接用作类型声明，并且SeaTunnel对类型关键字不区分大小写。 例如，如果您需要声明一个整数类型的字段，您可以简单地将字段定义为`int`或`"int"`。
...
- 声明array类型时，需要指定元素类型，类型定义遵循 `array<T>` 格式，其中 `T` 代表元素类型。元素类型包括`int`,`string`,`json`,`boolean`,`tinyint`,`smallint`,`bigint`,`float` 和 `double`。与十进制类型声明类似，它也用 `"` 括起来。例如，在声明具有整数数组的字段时，将字段类型指定为 `"array<int>"`。
