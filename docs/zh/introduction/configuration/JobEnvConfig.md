...
该计数器按 pipeline 维度统计，且在整个作业运行期间累计计数：每次尝试恢复都会自增，且永远不会被重置，即使某次恢复成功后 pipeline 已经稳定运行了很长时间也不会重置。例如，默认 `job.retry.times = 3` 时，如果 pipeline 失败并成功恢复了 3 次，之后再次失败将不会再重试——即使距离第 3 次恢复之后已经无故障运行了数小时，pipeline 也会直接永久失败。重试次数不会按单次故障单独计算，也不会因为一段时间的正常运行而重置；唯一能重置该计数器的方式是重新提交作业（开启新的一次作业运行）。
