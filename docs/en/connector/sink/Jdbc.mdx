...
benefit from param placeholder, `JdbcSink` supports upsert semantics through `insert into on duplicate ` or `replace into` grammer:
...
JdbcSink {
    ...
    url = "jdbc:mysql://localhost/test"
    username = root
    query = "insert into test(name,age) values(#{name}, #{age}) on duplicate key update name = #{name}, age = #{age}"
    batch_size = 2
}
```
