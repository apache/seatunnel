...
Cancellation deletes the Job and labeled Pods, Service, and ConfigMap. It does not create a savepoint. Status returns `UNKNOWN` after deletion or Job TTL expiry.

## How do I inspect resources and logs?

```bash
kubectl -n seatunnel-apps get jobs,pods,services,configmaps \
...
The master exit code produces a Job `Complete` or `Failed` condition. The completed master Pod, ConfigMap, and Service remain until `kubernetes.finished-job-retention-seconds` expires.
...
- Restrict Job ConfigMap access and do not expose secrets in configuration or logs.
