...
- If the value is a map type, you can pass it as a JSON string, for example:`-i mysql_properties='{"connectTimeout":"5000","connectionTimeZone":"UTC","serverTimezone":"UTC","useSSL":"false","allowPublicKeyRetrieval":"true"}'`. In this case, both keys and values, regardless of their data types, must be wrapped with `\"`. The curly braces must be enclosed in single quotes (') to prevent shell variable expansion. Nested JSON parameters are not supported yet.
