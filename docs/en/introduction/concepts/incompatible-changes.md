...
- **Breaking Change: Debezium runtime ownership moved out of `connector-cdc-base`**
  - **Affected component**: `seatunnel-connectors-v2/connector-cdc/*`
  - **Description**: `connector-cdc-base` now declares `debezium-api`, `debezium-embedded` and `zstd-jni` as `provided`, and each CDC connector declares the Debezium version it ships through its own `debezium.version` property. Previously every CDC connector inherited Debezium transitively from `connector-cdc-base`, so the shared `connector-cdc-base` jar was the single Debezium version for the whole project. Because `connector-cdc-base` is loaded into the class loader of every CDC connector, no connector could override it. The stock Debezium runtime now lives in each connector's own jar instead, while `connector-cdc-base` still keeps five patched `io.debezium` classes. Four of them (`ChangeEventQueue`, `TableId`, `HeartbeatFactory` and `HistorizedRelationalDatabaseConnectorConfig`) replace stock `debezium-core` classes; `DefaultHeartbeatConnectionProvider` is a SeaTunnel-only implementation and has no stock class to exclude.
  - **Impact**: No change for CDC job configuration, runtime behavior or checkpoint compatibility — every in-tree connector still resolves Debezium `1.9.8.Final`, so the set of classes on a CDC job's class loader is unchanged. Upgrade the `connector-cdc-base` jar and every deployed Debezium CDC connector jar as one set from the same SeaTunnel release; mixing pre-change and post-change plugin jars in one plugin directory is unsupported and can leave the runtime with a missing or duplicate Debezium class. Third-party CDC connectors built against `connector-cdc-base` and relying on it to supply Debezium transitively must now declare `debezium-api`, `debezium-embedded`, `zstd-jni`, and their connector-specific Debezium artifact themselves. When their package contains `io.debezium:debezium-core`, their `maven-shade-plugin` configuration must exclude the stock outer and nested classes for `ChangeEventQueue`, `TableId`, `HistorizedRelationalDatabaseConnectorConfig` and `HeartbeatFactory`; otherwise a patched base class and a stock connector class race on classpath order. A connector that declares its own shade filter must restate these exclusions. Do not exclude `DefaultHeartbeatConnectionProvider`, because it is supplied only by `connector-cdc-base`.
  ...
  - **How far connectors may actually diverge**: A connector may now resolve a different Debezium version, but `connector-cdc-base` is still *compiled* against a single baseline and is loaded into every CDC connector's class loader. It uses around 85 Debezium types across `io.debezium.relational`, `io.debezium.pipeline`, `io.debezium.schema`, `io.debezium.config` and `io.debezium.jdbc`, and it ships patched copies of `ChangeEventQueue`, `TableId`, `HeartbeatFactory`, `HistorizedRelationalDatabaseConnectorConfig` and `DefaultHeartbeatConnectionProvider`. Divergence is therefore safe only within a Debezium line that keeps those APIs binary compatible. Moving a connector to a Debezium release that changes them additionally requires `connector-cdc-base`'s Debezium usage to move behind the `DebeziumAdapter` SPI, which is not yet done.
