...
- **Behavior change: the REST log-content endpoints return at most 64 MB by default**
  ...
  - **Description**: These endpoints read the requested log file whole, which materialises it on the
    ...
    of content, starting at the first complete line, and the response opens with a line naming the
    returned and the total size.
  ...
  - **Migration Guide**: Set `seatunnel.engine.http.log-response-max-size-mb: 0` under
