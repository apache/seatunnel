...
- **Breaking Change: `schema.columns[].defaultValue` is now applied to missing or explicitly-`null` JSON fields**
  - **Affected component**: `seatunnel-format-json` (connectors that construct the deserializer from a `CatalogTable`: Kafka, Pulsar, HTTP, GraphQL, MQTT, RocketMQ, Amazon SQS, Google Sheets, RabbitMQ, Redis, the File connector outside merge-partition mode, and the Debezium/Canal/Ogg/Maxwell CDC wrappers; the File connector's merge-partition mode is unaffected because partition columns are resolved from the file path at read time and cannot be expressed through the catalog table)
