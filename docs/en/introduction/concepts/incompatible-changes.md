...
- **Breaking Change: Zeta master health and telemetry fields now report the active SeaTunnel coordinator**
  ...
  - **CLI member list**: `seatunnel-cluster.sh -m` follows the same rule. `ACTIVE MASTER` now marks the active SeaTunnel coordinator instead of the Hazelcast master, so a worker-only Hazelcast master is listed as `WORKER`. When the coordinator is inferred behind a worker-only Hazelcast master, the command prints an `Active master: <address> (best effort)` line because the client membership view can lag behind the cluster. When no coordinator can be resolved, it prints `Active master: UNKNOWN` and the `MASTER` rows only show the configured role.
  - **Failover window**: While a coordinator is being elected or while no coordinator-capable member is visible, no node reports `isMaster=true` and `cluster_info` is not exported. Alert rules must tolerate this short window instead of treating it as a lost cluster.
