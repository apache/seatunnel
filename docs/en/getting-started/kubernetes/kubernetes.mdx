...
```
Image `seatunnel:2.3.13-flink-1.13` needs to be present in the host (minikube) so that the deployment can take place.
...
```
Image `seatunnel:2.3.13` need to be present in the host (minikube) so that the deployment can take place.
...
docker build -t seatunnel:2.3.13 -f Dockerfile .
```
Image `seatunnel:2.3.13` needs to be present in the host (minikube) so that the deployment can take place.
...
In this guide we will use [seatunnel.streaming.conf](https://github.com/apache/seatunnel/blob/2.3.13-release/config/v2.streaming.conf.template):
