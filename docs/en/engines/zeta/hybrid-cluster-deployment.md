...
### 5.3.1 Compaction Configuration (Optional)

When IMap persistence is enabled, SeaTunnel writes WAL files continuously to external storage. Over time, this may produce many small files and duplicate records for the same key.

To optimize storage size and read performance, SeaTunnel provides an LSM-based compaction mechanism, which periodically merges multiple WAL files into larger sorted files and keeps only the latest record for each key.

Compaction is **disabled by default** and can be enabled per IMap.

### Configuration Options

You can configure compaction behavior in `map-store.properties`:
    ...
    engine*:
        map-store:
            ...
            properties:
                ...
                # Maximum total size processed in one compaction batch (bytes)
                compactionBatchSize: 16777216
                # Sleep interval between compaction batches (milliseconds)
...
Total WAL file size (in bytes) required to trigger compaction.

Default: 536870912 (512 MB)

**maxSingleFileSize [long]**

Maximum size of a single output file generated by compaction.

Default: 16777216 (16 MB)

**compactionBatchSize [long]**

Maximum total input size processed in one compaction round.
When exceeded, compaction will pause for compactionInterval.

Default: 16777216 (16 MB)

Note: This value must be greater than or equal to maxSingleFileSize.

**compactionInterval [long]**

Sleep time (in milliseconds) between compaction batches, used to limit IO pressure.
