...
Whether to fail a restored pipeline when either restore phase exceeds `restore-progress-timeout`. The default is `false`, which only records a warning with restore diagnostics.
