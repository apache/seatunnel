...
Hazelcast's `SlowOperationDetector` monitors the execution time of operations on partition threads. When an operation takes longer than the configured threshold (default: 10 seconds, configurable via `hazelcast.slow.operation.detector.threshold.millis`), a warning is logged:
...
- Check: Enable DEBUG logging for `org.apache.seatunnel.engine.server.checkpoint.CheckpointCoordinator` (checkpoint completion is logged at DEBUG level), then grep for `cost:` in the logs to see checkpoint durations.
...
**Configuration location:** `hazelcast.yaml` under the `hazelcast.properties` section:
...
| `seatunnel.engine.checkpoint.interval` | `seatunnel.yaml` | **Yes** (node restart) | Parsed once at node bootstrap; requires restart to take effect |
| `seatunnel.engine.checkpoint.timeout` | `seatunnel.yaml` | **Yes** (node restart) | Parsed once at node bootstrap; requires restart to take effect |
| `seatunnel.engine.checkpoint.storage.*` | `seatunnel.yaml` | **Yes** (node restart) | Parsed once at node bootstrap; requires restart to take effect |
| `seatunnel.engine.history-job-expire-minutes` | `seatunnel.yaml` | **Yes** (node restart) | Parsed once at node bootstrap; requires restart to take effect |
