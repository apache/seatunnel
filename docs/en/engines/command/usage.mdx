...
For Spark 3.5.x, use `bin/start-seatunnel-spark-3.5-connector-v2.sh` with the same options. Set `SPARK_HOME` to the matching Spark installation. See [Spark engine compatibility](../spark.md) for the current validation limits.
