...
| batch_interval         | int                 | No       | 1000                         | Write batch interval, it can be disabled if config a non-positive. Writing flush will be triggered when either `batch_size` or `batch_interval` is met. |
