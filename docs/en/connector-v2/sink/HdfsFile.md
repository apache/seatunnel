...
| max_rows_in_memory               | int     | no       | -                                          | When this parameter is empty, It means to keep 100 lines in memory, Only used when file_format is excel. |
...
For excel file format with `have_partition` and `custom_filename` and `sink_columns`
