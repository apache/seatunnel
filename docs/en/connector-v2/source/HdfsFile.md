...
| file_size_per_split                  | long    | no       | none                | -                                                                                                                                           |
| row_count_per_split                  | long    | no       | none                | -                                                                                                                                           |
...
whether to split a file into many splits. true will split. only valid for orc/parquet now

### file_size_per_split

split a file into many splits according to file size, if row_count_per_split not config. use row_count_per_split prefer. only valid for orc now. parquet file will auto split by block size.

### row_count_per_split

split a file into many splits according to row count. only valid for orc now. parquet file will auto split by block size.

### batch_read_rows

max size in a batch. now only useful for orc file. default is 1024, if memory is enough or row size is small, you can increase it to speed up reading.
