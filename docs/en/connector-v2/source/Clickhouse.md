...
For sql mode, the parallel reading is implemented based on the parallelism execution of local table-based queries on each shard of the cluster. If the sql parameter specifies a distributed table, the corresponding local table will be automatically converted to execute the query
...
If `sql` parameters are specified, the query will be executed only using sql.
...
source {
  Clickhouse {
    ...
    table_path = "default.table"
    server_time_zone = "UTC"
    sql = "select * from default.table where age = 20 limit 100"
    batch_size = 1024
    clickhouse.config = {
      "socket_timeout": "300000"
    }
