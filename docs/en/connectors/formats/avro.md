...
This format decodes plain Avro binary directly using the `schema` you configure; it does not talk to a schema registry. For Confluent Schema Registry messages consumed through Kafka, set `strip_schema_registry_header = true` and provide `avro_schema`. The connector validates and strips the five-byte header (magic byte `0` plus four-byte schema ID) without contacting Schema Registry. The option is opt-in and raw Avro behavior is unchanged when disabled.
