...
2. Change the REPLICA policy of the specified table to FULL, unless `require-replica-identity-full` is set to `false`. Jobs reading tables without `REPLICA IDENTITY FULL` fail at submit (and at checkpoint restore) unless one of the two is done.
...
| startup.mode                              | Enum     | No       | INITIAL  | Optional startup mode for Opengauss CDC consumer, valid enumerations are `initial`, `snapshot-only`, `earliest` and `latest`. <br/> `initial`: Synchronize historical data at startup, and then synchronize incremental data.<br/> `snapshot-only`: Synchronize historical data at startup and finish as a bounded job without entering WAL streaming.<br/> `earliest`: Startup from the earliest offset possible.<br/> `latest`: Startup from the latest offset.<br/> `committed-offset` is PostgreSQL-only (it reads `pg_replication_slots` columns OpenGauss does not expose) and is rejected at config validation for this connector.                                                                                                                                                                                                                                                                                                 |
