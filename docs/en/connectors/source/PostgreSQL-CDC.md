...
| schema-changes.enabled                    | Boolean  | No       | false    | Enable schema evolution events. When set to `true`, PostgreSQL CDC can forward `ADD COLUMN`, `DROP COLUMN`, `RENAME COLUMN`, and `ALTER COLUMN ... TYPE ...` events to downstream consumers.                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
> Schema evolution currently supports column add, drop, rename, and type change events only.
