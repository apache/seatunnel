...
| query                        | String     | No       | -       | SELECT statement used to read data. The column list of the SELECT defines the output schema; select only the columns you need. Required when neither `table_path` nor `table_list` is configured.                                              |
