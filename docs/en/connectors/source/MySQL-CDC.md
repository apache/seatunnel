...
| sample-sharding.enable                    | Boolean  | No       | true    | Whether to enable sampling-based sharding strategy. When set to false, the system should fall back to unevenly-sized chunk splitting (iterative query approach) regardless of the shard count. The default value is true. |
