...
| scan.newly-added-table.enabled            | Boolean  | No       | true    | Whether to scan tables newly captured by `database-pattern` or `table-pattern` after restoring from a checkpoint or savepoint. This keeps the existing SeaTunnel restore behavior by default.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| scan.binlog.newly-added-table.enabled     | Boolean  | No       | false   | Whether to register and read newly created tables during the binlog reading phase. This option starts from the `CREATE TABLE` binlog position and does not snapshot historical data for those tables. The new table must match the configured capture pattern, and the downstream sink must be able to route or create the target table.                                                                                                                                                                                                                                                                               |
...
`scan.newly-added-table.enabled` is evaluated only when the job is restored from a checkpoint or savepoint. After restore, SeaTunnel compares the current captured tables with the checkpointed tables. Newly matched tables are added to the snapshot phase and historical data is read before the job continues to the binlog phase.

`scan.binlog.newly-added-table.enabled` works while the job is already reading binlog. It registers table metadata from the `CREATE TABLE` schema record and then reads following DML records for that table. It does not backfill rows that existed before the `CREATE TABLE` binlog position. If the sink cannot create or route the new target table, the job can still fail downstream.
