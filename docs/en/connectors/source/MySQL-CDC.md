...
| exactly_once                              | Boolean  | No       | false   | Enable exactly once semantics. When `startup.mode` is `initial` or `snapshot`, it additionally enables bounded low-to-high-watermark binlog backfill during the snapshot phase.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Because the job never enters continuous binlog streaming, `snapshot` mode is incompatible with `stop.mode` values other than `never` and the binlog-oriented startup offset options `startup.specific-offset.*` and `startup.timestamp`; configuring them together fails fast at startup. `exactly_once` remains available for bounded backfill during the snapshot phase and does not start continuous binlog streaming.
