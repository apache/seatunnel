...
| startup.mode                              | Enum     | No       | INITIAL | Optional startup mode for MySQL CDC consumer, valid enumerations are `initial`, `earliest`, `latest` , `specific`, `timestamp` and `mixed`. <br/> `initial`: Synchronize historical data at startup, and then synchronize incremental data.<br/> `earliest`: Startup from the earliest offset possible.<br/> `latest`: Startup from the latest offset.<br/> `specific`: Startup from user-supplied specific offsets.<br/> `timestamp`: Startup from user-supplied timestamp.<br/> `mixed`: Snapshot the tables in `startup.snapshot-table-names`; all other configured tables start from the specific binlog offset. |
| startup.snapshot-table-names              | List     | Conditionally required | -       | Tables initialized by a full snapshot when `startup.mode` is `mixed`. Every value must also be listed in `table-names`; every remaining table starts from `startup.specific-offset.*`.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| startup.specific-offset.file              | String   | No       | -       | Start from the specified binlog file name. **Note, This option is required when the `startup.mode` option used `specific` or `mixed`.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| startup.specific-offset.pos               | Long     | No       | -       | Start from the specified binlog file position. **Note, This option is required when the `startup.mode` option used `specific` or `mixed`.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| startup.specific-offset.gtid-set          | String   | No       | -       | Optional MySQL GTID set for `specific` or `mixed` startup mode. This option is used together with `startup.specific-offset.file` and `startup.specific-offset.pos`.                                                                                                                                                                                                                                                                                                                                                                                |
| startup.specific-offset.skip-events       | Long     | No       | 0       | Number of binlog events to skip after the configured specific startup offset. This option can only be used when `startup.mode` is `specific` or `mixed`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| startup.specific-offset.skip-rows         | Long     | No       | 0       | Number of rows to skip after the configured specific startup offset. This option can only be used when `startup.mode` is `specific` or `mixed`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| incremental.parallelism                   | Integer  | No       | 1       | The number of parallel readers in the incremental (binlog) phase.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
### Snapshot Some Tables and Start Other Tables From a Binlog Offset

Use `startup.mode = "mixed"` when one source must snapshot selected tables while the remaining
configured tables start from one common binlog file and position. The selected snapshot tables
continue with the existing high-watermark filtering, so their snapshot rows are not duplicated by
the final binlog reader. Tables outside `startup.snapshot-table-names` emit change records only at
or after the configured specific offset.

This first version requires `table-names` (not `table-pattern`), `exactly_once = true`, and
`incremental.parallelism = 1`. The final reader starts at the earliest offset needed by either
policy, so MySQL binlog retention must cover the whole snapshot duration from the configured
specific offset. The table set, the snapshot-table selection, and the specific offset are fixed for
the job; changing any of them before checkpoint restore fails the restore instead of silently
