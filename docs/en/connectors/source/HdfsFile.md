...
| date_format | string | no | - | Date type format, used to tell connector how to convert string to date. When not configured, the connector auto-detects the format from unambiguous candidates such as `yyyy-MM-dd`, `yyyy.MM.dd`, `yyyy/MM/dd`, `yyyyMMdd`. |
| datetime_format | string | no | - | Datetime type format, used to tell connector how to convert string to datetime. When not configured, the connector auto-detects the format from standard year-first patterns such as `yyyy-MM-dd HH:mm:ss`, `yyyy/MM/dd HH:mm:ss`, `yyyyMMddHHmmss`. |
| time_format | string | no | - | Time type format, used to tell connector how to convert string to time. When not configured, the connector auto-detects the format from `HH:mm:ss`, `HH:mm:ss.SSS`, `H:mm:ss`, `H:mm`. |
