...
FakeSource is a virtual source that generates rows from a user-defined schema. It is mainly used for demos, connector tests, type conversion checks, and local pipeline verification.
...
|-------------------------|----------|----------|-------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| tables_configs          | list     | No       | -                       | Defines multiple FakeSource tables. Each item can contain the same options as a single FakeSource block, including its own schema and rows. Use either `tables_configs` or top-level `schema`. |
| schema                  | config   | conditionally yes | -                 | Defines schema information. Required when `tables_configs` is not configured. For more details, see [Schema Feature](../../introduction/concepts/schema-feature.md). |
| auto.increment.enabled  | boolean  | No       | false                   | Enable auto increment ID generation                                                                                                                                                                            |
| auto.increment.start    | long     | No       | 1                       | Starting value for auto increment ID. This option only takes effect when `auto.increment.enabled` is `true`. |
...
| split.read-interval     | int      | No       | 1                       | The interval in milliseconds between two split reads in a reader.                                                                                                                               |
...
| int.fake.mode           | string   | No       | range                   | The fake mode of generating int data, support `range` and `template`, default `range`. If set to `template`, configure `int.template` too. |
