...
| split.assign.batch-size                    | Int     | No        | 16              | Maximum number of splits assigned to each reader per handoff. Readers request more splits when their local queue runs low. This bounds reader-side pending split metadata and checkpoint size without changing split predicates. |
| split.max-pending-splits                   | Int     | No        | 64              | Maximum number of already-generated but not-yet-assigned splits retained by the enumerator. The enumerator lazily generates the next splits when pending work falls below this watermark. This bounds coordinator memory without capping the total number of splits for the job. |
| split.even-distribution.factor.lower-bound | Double  | No        | 0.05            | Not recommended for use.<br/> The lower bound of the chunk key distribution factor. This factor is used to determine whether the table data is evenly distributed. If the distribution factor is calculated to be greater than or equal to this lower bound (i.e., (MAX(id) - MIN(id) + 1) / row count), the table chunks would be optimized for even distribution. Otherwise, if the distribution factor is less, the table will be considered as unevenly distributed and **index probing** (iterative `queryNextChunkMax`) is used to place chunk boundaries. The default value is 0.05.  |
| split.even-distribution.factor.upper-bound | Double  | No        | 100             | Not recommended for use.<br/> The upper bound of the chunk key distribution factor. This factor is used to determine whether the table data is evenly distributed. If the distribution factor is calculated to be less than or equal to this upper bound (i.e., (MAX(id) - MIN(id) + 1) / row count), the table chunks would be optimized for even distribution. Otherwise, if the distribution factor is greater, the table will be considered as unevenly distributed and **index probing** is used. The default value is 100.0. |
| split.sample-sharding.threshold            | Int     | No        | 1000            | Retained for compatibility and for bounding sparse-range fallback when approximate row count is unavailable. Uneven dynamic splitting no longer runs full-column client sampling on the live planning path; it uses index probing instead. The default value is 1000 shards.                                                                                                                 |
| split.inverse-sampling.rate                | Int     | No        | 1000            | Retained for compatibility with older sampling-oriented configurations. Full-column client sampling is not used by the live dynamic-split planner. The default value is 1000.                                                                                                                                                                                            |
| split.allow-sampling                       | Boolean | No        | true            | Retained for compatibility. Full-column client sampling is no longer reachable on the live dynamic-split planning path; uneven keys use index probing regardless of this flag.                                                                                                                                                              |
...
Dynamic splitting generates splits lazily for numeric and non-charset string keys: even keys use an arithmetic cursor, uneven keys use index probing, and the enumerator only keeps up to `split.max-pending-splits` generated splits before assigning batches of `split.assign.batch-size` to readers. Raise those two options if readers idle between handoffs; they do not raise a hard cap on the total number of splits for the job. DATE split keys and charset-based STRING split keys still pre-materialize their full split list in this release.
