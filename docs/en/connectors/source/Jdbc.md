...
When only `query` is configured and the query maps to a single physical table (for example `SELECT * FROM db.table`), SeaTunnel resolves the underlying table from the query result metadata and merges its metadata — column and table comments, primary key, constraint keys, and partition keys — into the query-derived schema, as if `table_path` had been configured together with `query`. Columns renamed or transformed in the query keep the query-derived definition, and the merge is skipped when the underlying table cannot be resolved.
