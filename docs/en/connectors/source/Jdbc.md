...
| split.string-strategy                      | String  | No       | -               | Controls how String partition columns are split. Available values are `none`, `hash`, `range`, and `auto`. `range` currently requires MySQL binary collation and fixed-length printable ASCII key values. `auto` tries range splitting first and falls back to hash splitting when range splitting is unsafe. When this option is not set, SeaTunnel keeps the existing `split.string_split_mode` behavior.                                                                                                                                                                                                                                           |
