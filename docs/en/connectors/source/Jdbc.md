...
| split.sample-sharding.threshold            | Int     | No        | 1000            | Retained for compatibility and for bounding sparse-range fallback when approximate row count is unavailable. Uneven dynamic splitting no longer automatically triggers full-column client sampling when the estimated shard count exceeds this threshold; it uses index probing instead. The default value is 1000 shards.                                                                                                                 |
| split.inverse-sampling.rate                | Int     | No        | 1000            | Retained for compatibility with older sampling-oriented configurations. Large-table dynamic splitting defaults to index probing rather than full-column client sampling. The default value is 1000.                                                                                                                                                                                            |
| split.allow-sampling                       | Boolean | No        | true            | Retained for compatibility. Large-table uneven dynamic splitting defaults to index probing and no longer automatically runs full-column client sampling; set documentation expectations accordingly.                                                                                                                                                              |
...
Dynamic splitting generates splits lazily: even keys use an arithmetic cursor, uneven keys use index probing, and the enumerator only keeps up to `split.max-pending-splits` generated splits before assigning batches of `split.assign.batch-size` to readers. Raise those two options if readers idle between handoffs; they do not raise a hard cap on the total number of splits for the job.
