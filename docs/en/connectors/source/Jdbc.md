...
| split.string-strategy                      | String  | No        | -               | Controls how String partition columns are split. Available values are `none`, `hash`, `range`, and `auto`. `range` is enabled only after the dialect validates its ordering contract: MySQL requires binary column collation and fixed-length printable ASCII key values; Oracle requires `NLS_SORT=BINARY` and `NLS_COMP=BINARY` plus the same key restriction. `range` rejects unsupported or unsafe configurations. `auto` uses range when validation succeeds and otherwise falls back to hash splitting, or a single split when hash is unavailable. When this option is not set, SeaTunnel keeps the existing `split.string_split_mode` behavior.                                                                                                                                                                                                                                           |
