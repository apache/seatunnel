...
When only `query` is configured and JDBC reports every result column as originating from the same physical table (for example `SELECT * FROM db.table`), SeaTunnel resolves that table from the query result metadata and merges its metadata — column and table comments, primary key, constraint keys, and partition keys — into the query-derived schema, as if `table_path` had been configured together with `query`. Columns renamed or transformed in the query keep the query-derived definition. The merge is skipped for detected multi-table queries and whenever the driver does not provide enough origin metadata to verify a single table. Existing query key inference has separate limitations; see [Query and primary-key caution](#query-and-primary-key-caution).
