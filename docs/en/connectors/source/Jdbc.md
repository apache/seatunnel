...
| query_table_metadata_merge                 | Enum    | No        | COMMENT         | How much metadata of the underlying physical table is merged into the schema derived from a query-only source table (only applies when the query is verified to map to a single physical table). `COMMENT` merges the column comments, table comment and table options. `ALL` additionally merges the primary key, constraint keys and partition keys — the same result as configuring `table_path` together with `query`; this may switch sinks with `generate_sink_sql` from insert to upsert and enable primary-key based split planning. `NONE` skips resolving the underlying table entirely. Can also be set per table in `table_list`.        |
