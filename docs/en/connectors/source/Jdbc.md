...
| split.string-strategy                      | String  | No        | -               | Controls how String partition columns are split. Available values are `none`, `hash`, `range`, and `auto`. `range` is enabled only after the dialect validates its ordering contract: MySQL requires binary column collation and fixed-length printable ASCII key values; Oracle requires `NLS_SORT=BINARY`, `NLS_COMP=BINARY`, a `BINARY` or `USING_NLS_COMP` column collation, database character encoding that preserves printable ASCII binary ordering, and the same key restriction. Oracle revalidates its session contract on every reader connection; an unsafe reader session fails closed before executing range predicates. `range` rejects unsupported or unsafe configurations. For a dialect that supports range splitting, `auto` uses range when validation succeeds and otherwise falls back to hash splitting, or a single split when hash is unavailable. Dialects without validated range support reject `range` and `auto`. When this option is not set, SeaTunnel keeps the existing `split.string_split_mode` behavior. |
