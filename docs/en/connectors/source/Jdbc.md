...
When only `query` is configured and JDBC reports every result column as originating from the same physical table (for example `SELECT * FROM db.table`), SeaTunnel resolves that table from the query result metadata and merges its metadata — column and table comments, primary key, constraint keys, and partition keys — into the query-derived schema, as if `table_path` had been configured together with `query`. Columns renamed in the query keep the query-derived definition. Note that a computed or expression column (for example `UPPER(name)`) usually reports no origin metadata, which leaves the whole query unverifiable — the merge is then skipped entirely, not only for that column. The merge is likewise skipped for detected multi-table queries, and any verification or load failure falls back to the query-derived schema with a warning log. Existing query key inference has separate limitations; see [Query and primary-key caution](#query-and-primary-key-caution).
