...
For tables with a **composite primary key** (`PRIMARY KEY (a, b, ...)`), the dynamic splitter splits on **all key columns** as a tuple (e.g. `(a, b) > (?, ?)`, emitted in a portable expanded `OR`/`AND` form), so tables whose first key column repeats heavily still split into balanced chunks via the remaining key columns. This applies when the dynamic splitter is active (the default), the dialect enables composite splitting, and all key columns are supported split-key types; otherwise the connector falls back to splitting on the first supported primary-key column as before. Composite splitting is currently enabled for **MySQL, PostgreSQL, SQLite, SQL Server, and Oracle**. The boundary queries use the `FETCH FIRST`/`OFFSET` pagination syntax, which the connector detects at runtime: on Oracle releases before 12c the connector automatically falls back to the single-column split; other dialects keep the single-column behavior.

Rows whose composite key contains a `NULL` component (possible on SQLite, which permits `NULL` inside composite primary keys) are excluded from chunk boundary computation and are assigned to the **first chunk** by an explicit predicate, so every row — including `NULL`-key rows — is read exactly once.

**Opting out of composite splitting:** to keep the previous single-column split behavior on a table with a composite primary key, set `partition_column` to a single primary-key column (e.g. the first key column). An explicit `partition_column` always takes precedence over the composite primary-key branch, so the table is split on that single column exactly as before composite splitting was introduced. Setting `enable_concurrent_read = false` alternatively disables split analysis entirely and reads the table with a single split.

**Known limitation — string collation:** the composite split path assumes that the database-side `ORDER BY` and the Java-side tuple comparison agree. STRING components are compared with Java `String.compareTo` (Unicode code-point) semantics, which may disagree with case- or accent-insensitive database collations (e.g. MySQL `utf8mb4_general_ci`). Composite primary keys whose STRING columns use a collation that diverges from binary/code-point ordering are not supported for the STRING component: chunk boundaries computed in Java may not match the row order returned by the database, which can duplicate or drop rows across those chunks. For such tables, prefer a numeric leading key column or fall back to a single-column split via `partition_column`.
