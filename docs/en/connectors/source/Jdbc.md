...
| Control selected columns, joins, or database-side expressions | `query`, optionally with `table_path` | SeaTunnel executes the SQL you provide. For a query whose JDBC metadata identifies one physical table, SeaTunnel preserves matching column and table comments automatically. Add `table_path` for joins, views, expressions, or any query where explicit table identity and metadata are needed. Query key inference is unsafe for some joins; see [Query and primary-key caution](#query-and-primary-key-caution). |
...
| query                                      | String  | No        | -               | SQL query to execute. For a query identified as one physical table, matching table and column comments are retained automatically. It can be combined with `table_path` when explicit table identity and metadata are also needed.                                                                                                                                                                                                                                                                                                                                                                                                             |
