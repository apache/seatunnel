...
| split.assign.batch-size                    | Int     | No        | 16              | Maximum number of splits assigned to each reader in one handoff. Readers request more splits when their local queue runs low. This bounds reader pending split metadata and checkpoint size without changing split predicates.                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
`split.assign.batch-size` controls how many generated splits are handed to each reader at a time. Remaining splits stay in the enumerator until the reader requests more. This does not change split boundaries or data coverage.
