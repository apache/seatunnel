...
When only `query` is configured and JDBC reports every result column as originating from the same physical table (for example `SELECT * FROM db.table`), SeaTunnel resolves that table from the query result metadata and, by default, merges its column comments and table comment into the query-derived schema, so sinks with automatic table creation preserve them. The default merge deliberately excludes table options, primary keys, constraint keys and partition keys because carrying them over can change generated sink DDL, switch a sink with `generate_sink_sql` from plain inserts to upserts, or make split planning infer a split column from the merged primary key. Set `query_table_metadata_merge = ALL` to merge them as well — the same result as configuring `table_path` together with `query` — or `NONE` to skip resolving the underlying table entirely. Columns renamed in the query keep the query-derived definition. Note that a computed or expression column (for example `UPPER(name)`) usually reports no origin metadata, which leaves the whole query unverifiable — the merge is then skipped entirely, not only for that column. The merge is likewise skipped for detected multi-table queries, and any verification or load failure falls back to the query-derived schema with a warning log. Existing query key inference has separate limitations; see [Query and primary-key caution](#query-and-primary-key-caution).
...
| query_table_metadata_merge                 | Enum    | No        | COMMENT         | How much metadata of the underlying physical table is merged into the schema derived from a query-only source table (only applies when the query is verified to map to a single physical table). `COMMENT` merges only the column comments and table comment. `ALL` additionally merges table options, the primary key, constraint keys and partition keys — the same result as configuring `table_path` together with `query`; this may change generated sink DDL, switch sinks with `generate_sink_sql` from insert to upsert and enable primary-key based split planning. `NONE` skips resolving the underlying table entirely. Can also be set per table in `table_list`.        |
