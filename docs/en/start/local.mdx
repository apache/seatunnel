...
````
Usually we don't need all the connector plugins, so you can specify the plugins you need by configuring `config/plugin.properties`, for example, you only need the `flink-assert` plugin, then you can modify plugin.properties as
````properties
--flink-connectors--
