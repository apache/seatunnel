...
<Tabs
    groupId="env-type"
    defaultValue="Unix/Linux"
    values={[
        {label: 'Windows', value: 'windows'},
        {label: 'Unix/Linux', value: 'Unix/Linux'}
    ]}>
<TabItem value="Unix/Linux">
...
```cmd
cd "apache-seatunnel-${version}"
call .\bin\seatunnel.cmd --config ./config/v2.batch.config.template -m local
...
## Step 4: Run SeaTunnel Application With a Local Cluster

First, run a local cluster with a default cluster name.
...
Or, Use a customer cluster name.
...
```shell
./bin/seatunnel-cluster.sh -cn customer-cluster
```
</TabItem>
...
```cmd
call .\bin\seatunnel-cluster.cmd -cn customer-cluster
```
</TabItem>
</Tabs>

After that, you can submit job to the cluster.
...
Stop cluster with bellow command.
...
```shell
./bin/stop-seatunnel-cluster.cmd
```
</TabItem>

<TabItem value="windows">

Stop cluster in windows need `wmic` command, please flow [this guide](https://techcommunity.microsoft.com/blog/windows-itpro-blog/how-to-install-wmic-feature-on-demand-on-windows-11/4189530) to install it if your windows doesn't have it.

```cmd
call .\bin\stop-seatunnel-cluster.cmd
```
</TabItem>
</Tabs>
