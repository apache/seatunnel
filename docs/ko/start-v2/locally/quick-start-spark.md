...
먼저 [Spark 다운로드](https://spark.apache.org/downloads.html) 페이지에서 **2.4.0 이상** 버전을 설치하세요. 자세한 안내는 [Standalone 설치](https://spark.apache.org/docs/latest/spark-standalone.html#installing-spark-standalone-to-a-cluster) 문서를 참고하면 됩니다.

**SeaTunnel 구성**: `${SEATUNNEL_HOME}/config/seatunnel-env.sh` 파일에서 `SPARK_HOME`을 Spark 설치 경로로 설정하세요.

## 3단계: 작업을 정의하는 설정 파일 추가

SeaTunnel이 시작된 뒤 데이터 입력·처리·출력 방식을 결정하는 파일 `config/seatunnel.streaming.conf.template`를 수정합니다.
...
- Spark에서 SeaTunnel을 사용하는 방법은 [SeaTunnel With Spark](../../other-engine/spark.md) 문서를 참고하세요.
