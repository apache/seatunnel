...
/** Test for {@link ClientExecuteCommand} fatal error handling. */
@Slf4j
public class ClientExecuteCommandTest {

    /**
     * This test verifies that System.exit(1) is called when a fatal error occurs during Hazelcast
     * instance initialization in createServerInLocal.
     */
    @Test
    public void testFatalErrorHandlingInCreateServerInLocal() throws Exception {
        ...
        try (MockedStatic<HazelcastInstanceFactory> mockedStatic =
            ...
            // Create a command
            ClientExecuteCommand command = new ClientExecuteCommand(new ClientCommandArgs());
                            ...
                            () -> {
                                try {
                                    ...
                                    java.lang.reflect.Method method =
                                            ClientExecuteCommand.class.getDeclaredMethod(
                                                    "createServerInLocal",
                                                    String.class,
                                                    SeaTunnelConfig.class);
                                    method.setAccessible(true);
                                    method.invoke(command, "test-cluster", new SeaTunnelConfig());
                                } catch (Exception e) {
     ...
     * Hazelcast instance initialization in createServerInLocal.
     */
    @Test
    public void testOutOfMemoryErrorHandlingInCreateServerInLocal() throws Exception {
        // Use Mockito to mock the static HazelcastInstanceFactory.newHazelcastInstance method
        try (MockedStatic<HazelcastInstanceFactory> mockedStatic =
                Mockito.mockStatic(HazelcastInstanceFactory.class)) {
