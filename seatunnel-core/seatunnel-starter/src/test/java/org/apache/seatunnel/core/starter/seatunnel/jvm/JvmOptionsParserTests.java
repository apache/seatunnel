...
public class JvmOptionsParserTests {
    ...
    private void assertExpectedJvmOptions(final int javaMajorVersion, final BufferedReader br, final List<String> expectedJvmOptions) {
        ...
        JvmOptionsParser.parse(javaMajorVersion, br, jvmOption -> {
            ...
            Assertions.assertFalse(seen.get(), "saw JVM option [" + jvmOption + "] more than once");
            seen.set(true);
        }, (lineNumber, line) -> Assertions.fail("unexpected invalid line [" + line + "] on line number [" + lineNumber + "]"));
        for (final Map.Entry<String, AtomicBoolean> seenJvmOption : seenJvmOptions.entrySet()) {
            Assertions.assertTrue(seenJvmOption.getValue().get(), "expected JVM option [" + seenJvmOption.getKey() + "]");
        }
    }
    ...
    public void testInvalidLines() throws IOException {
        try (StringReader sr = new StringReader("XX:+UseG1GC");
            ...
            JvmOptionsParser.parse(
                ...
                br,
                jvmOption -> Assertions.fail("unexpected valid JVM option [" + jvmOption + "]"), (lineNumber, line) -> {
