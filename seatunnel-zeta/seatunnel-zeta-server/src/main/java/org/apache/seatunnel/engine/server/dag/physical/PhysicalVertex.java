...
import org.apache.seatunnel.zeta.common.Constant;
...
import org.apache.seatunnel.zeta.server.SeaTunnelServer;
import org.apache.seatunnel.zeta.server.dag.execution.ExecutionVertex;
import org.apache.seatunnel.zeta.server.execution.ExecutionState;
import org.apache.seatunnel.zeta.server.execution.TaskExecutionState;
...
import org.apache.seatunnel.zeta.server.master.JobMaster;
import org.apache.seatunnel.zeta.server.resourcemanager.resource.SlotProfile;
import org.apache.seatunnel.zeta.server.task.TaskGroupImmutableInformation;
import org.apache.seatunnel.zeta.server.task.operation.CancelTaskOperation;
import org.apache.seatunnel.zeta.server.task.operation.CheckTaskGroupIsExecutingOperation;
import org.apache.seatunnel.zeta.server.task.operation.DeployTaskOperation;
 ...
 * Each {@link org.apache.seatunnel.zeta.server.dag.execution.ExecutionVertex} generates some PhysicalVertex.
...
public class PhysicalVertex {
    ...
    private boolean checkTaskGroupIsExecuting(TaskGroupLocation taskGroupLocation){
        IMap<PipelineLocation, Map<TaskGroupLocation, SlotProfile>> ownedSlotProfilesIMap = nodezeta.getHazelcastInstance().getMap(Constant.IMAP_OWNED_SLOT_PROFILES);
        ...
        if (null != slotProfile){
            ...
            List<Address> members = nodezeta.getClusterService().getMembers().stream().map(Member::getAddress)
            ...
            InvocationFuture<Object> invoke = nodezeta.getOperationService().createInvocationBuilder(
    ...
    private void deployOnLocal(@NonNull SlotProfile slotProfile) {
        deployInternal(taskGroupImmutableInformation -> {
            SeaTunnelServer server = nodezeta.getService(SeaTunnelServer.SERVICE_NAME);
    ...
    private void deployOnRemote(@NonNull SlotProfile slotProfile) {
        deployInternal(taskGroupImmutableInformation -> {
            try {
                nodezeta.getOperationService().createInvocationBuilder(Constant.SEATUNNEL_SERVICE_NAME,
                        new DeployTaskOperation(slotProfile,
                            nodezeta.getSerializationService().toData(taskGroupImmutableInformation)),
    ...
    public void deploy(@NonNull SlotProfile slotProfile) {
        try {
            if (slotProfile.getWorker().equals(nodezeta.getThisAddress())) {
    ...
    private TaskGroupImmutableInformation getTaskGroupImmutableInformation() {
        return new TaskGroupImmutableInformation(flakeIdGenerator.newId(),
            nodezeta.getSerializationService().toData(this.taskGroup),
            this.pluginJarsUrls);
    }
    ...
    private void noticeTaskExecutionServiceCancel() {
        ...
        while (!taskFuture.isDone() && nodezeta.getClusterService().getMember(getCurrentExecutionAddress()) != null) {
            try {
                i++;
                LOGGER.info(
