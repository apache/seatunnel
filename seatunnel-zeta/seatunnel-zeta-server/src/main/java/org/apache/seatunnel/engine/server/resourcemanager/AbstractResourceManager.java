...
package org.apache.seatunnel.zeta.server.resourcemanager;

import org.apache.seatunnel.zeta.common.Constant;
import org.apache.seatunnel.zeta.common.runtime.ExecutionMode;
import org.apache.seatunnel.zeta.server.resourcemanager.opeartion.ReleaseSlotOperation;
import org.apache.seatunnel.zeta.server.resourcemanager.opeartion.ResetResourceOperation;
import org.apache.seatunnel.zeta.server.resourcemanager.resource.ResourceProfile;
import org.apache.seatunnel.zeta.server.resourcemanager.resource.SlotProfile;
import org.apache.seatunnel.zeta.server.resourcemanager.worker.WorkerProfile;
...
public abstract class AbstractResourceManager implements ResourceManager {
    ...
    public AbstractResourceManager(Nodezeta nodezeta) {
        this.registerWorker = nodezeta.getHazelcastInstance().getMap(Constant.IMAP_RESOURCE_MANAGER_REGISTER_WORKER);
    ...
    private void checkRegisterWorkerStillAlive() {
        if (!registerWorker.isEmpty()) {
            List<Address> aliveWorker = nodezeta.getClusterService().getMembers().stream().map(Member::getAddress)
    ...
    protected <E> InvocationFuture<E> sendToMember(Operation operation, Address address) {
        return NodezetaUtil.sendOperationToMemberNode(nodezeta, operation, address);
    }
    ...
    public CompletableFuture<Void> releaseResource(long jobId, SlotProfile profile) {
        if (nodezeta.getClusterService().getMember(profile.getWorker()) != null) {
